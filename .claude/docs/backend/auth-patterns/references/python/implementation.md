# Auth Patterns — реализация на Python (FastAPI / PyJWT / authlib)

Реализация язык-нейтрального контракта `../spec.md` (`AUTH-*`) на FastAPI. Коды общие с Java;
...
downstream. `auth-patterns/checks-split-by-layer` — BFF/application: грубая RBAC по роли — FastAPI-зависимость `Depends(require_roles("admin"))`.
`auth-patterns/checks-split-by-layer` — domain-handler: ABAC по ресурсу (`order.customer_id == principal.sub`), не на gateway (он не знает домен).

## 2. JWT validation (`AUTH-4..6`)

`auth-patterns/token-validated-by-library` — JWT валидируется одной общей зависимостью на проверенной библиотеке (`PyJWT` + `PyJWKClient` / `authlib`
ResourceProtector / `joserfc`), не россыпью самописного `jwt.decode` без проверки подписи/claims. `auth-patterns/token-validated-by-library` — JWK Set
тянется из IdP по `jwks_uri` с кешем (~5 мин, `PyJWKClient` кеширует; не распаковывать ключи руками). `auth-patterns/401-not-403` —
...
просроченный `exp` → **401** (`auth-patterns/401-not-403`). RBAC — `Depends(require_roles(...))` на каждом endpoint (`auth-patterns/every-endpoint-has-role-check`):
...
`auth-patterns/roles-from-token-claims` — роли из claim (`realm_access.roles` Keycloak / `scope`) маппятся в `Principal.roles` в зависимости.
`auth-patterns/roles-from-token-claims` — разрешённые роли: `customer`/`seller`/`admin`/`system`. `auth-patterns/every-endpoint-has-role-check` — на каждом endpoint —
`Depends(require_roles(...))`; endpoint без проверки роли — критично.

## 4. ABAC (`AUTH-10..12`)

`auth-patterns/ownership-checked-in-domain` — команда/запрос с агрегатом по id — ABAC по владению: сравнение `aggregate.owner_id` с `principal.sub` в
Handler с `403`/`ForbiddenError`. `auth-patterns/ownership-checked-in-domain` — ABAC-логика в выделенном компоненте (`AccessPolicy`)/Handler, не
размазана по роутерам. `auth-patterns/admin-bypass-is-audited` — `admin` обходит ABAC, но каждое действие — в audit log (`auth-patterns/admin-commands-write-audit-log`).
...
запроса (`usecase-pattern/no-transport-objects-in-usecase`). `admin` обходит проверку, но действие пишется в audit log (`auth-patterns/admin-bypass-is-audited`, `auth-patterns/admin-commands-write-audit-log`):
...
`scope=service:operation`). `auth-patterns/service-to-service-authenticated` — outbound-клиенты в `adapters/out/*` не ходят без mTLS/`Bearer`; анонимный
...
`auth-patterns/no-pii-in-logs-and-events` — PII (email/phone/ФИО/адрес) не в логах (даже DEBUG), не в `str(exc)`/`problem.detail`, не в Kafka-событиях
(только id, PII подгружает потребитель) — cross-ref `observability/no-pii-in-logs`. `auth-patterns/no-secrets-in-repository` — секреты (client-secret, DB-пароли,
ключи) **не в git** — через env / Vault / SealedSecrets, читаются `pydantic-settings`. `auth-patterns/error-response-hides-cause` — exception-handler
не выводит `str(cause)` в `detail` — только заранее заданное сообщение по коду (cross-ref `R-ERR-MAP-*`).

Секреты и параметры IdP — через `pydantic-settings` из env/Vault, не хардкодом и не в git (`auth-patterns/no-secrets-in-repository`). `SecretStr`
не утекает в логи/repr. Exception-handler отдаёт фиксированное сообщение по коду, без `str(cause)` (`auth-patterns/error-response-hides-cause`):
...
`auth-patterns/money-commands-need-idempotency-key` — команда, меняющая деньги/резерв (`CreateOrder`, `ConfirmPayment`, `Refund`), требует `Idempotency-Key`;
повтор с тем же ключом → прежний результат, не дубль (cross-ref `distributed/http-idempotency-record`).
...
`localStorage`. `auth-patterns/refresh-token-rotation` — refresh-токены с rotation: при обновлении старый инвалидируется; повторное использование
