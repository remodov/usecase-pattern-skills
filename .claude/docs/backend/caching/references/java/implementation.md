...
Свод правил кеширования в Java/Spring-сервисах команды UCP: что кешируем (что НЕ кешируем — чаще), Spring Cache + Redis конфигурация, key/TTL/invalidation, cache-aside vs write-through, защита от cache stampede, observability. Каждое правило идентифицируется кодом (`caching/cache-read-heavy-stable-data`, `caching/no-routine-full-flush`) — скилл `ucp-caching-review` цитирует эти коды в findings.

Гайд опирается на [Spring Cache abstraction](https://docs.spring.io/spring-framework/reference/integration/cache.html) с Redis backend через `spring-boot-starter-data-redis`. ConcurrentMapCacheManager (in-memory) — допустим только в тестах. Не покрывает: кеш на стороне клиента (HTTP `Cache-Control` — это `resilience/fallback-does-not-fake-success`/REST), database-level query plan cache (это PG-side), CDN.

Связанные стандарты:
- `resilience/fallback-does-not-fake-success` (`resilience/fallback-does-not-fake-success`) (Resilience) — fallback может отдавать cached read при отказе внешней системы.
- `auth-patterns/no-pii-in-logs-and-events` (`auth-patterns/no-pii-in-logs-and-events`) (Auth) — PII в кеше требует отдельной политики (TTL, encryption at rest).
- `jooq/nested-collections-in-one-query` (`jooq/nested-collections-in-one-query`) (jOOQ) — кеш не отменяет multiset для eager-fetch, это разные слои защиты.
...
- **R-CACHE-WHERE-X5.** Кеш **результата валидации / авторизации**. JWT-валидация имеет встроенный JWK кеш (5 минут, см. `auth-patterns/token-validated-by-library`); ABAC-проверки делаются каждый раз — кеш авторизации = security risk при изменении roles.
...
- **R-CACHE-CFG-4.** Cache settings — через `@ConfigurationProperties` (см. `validation/config-validated-at-startup`):
  ```java
...
- **R-CACHE-CFG-X1.** `JdkSerializationRedisSerializer` — security risk, см. `caching/values-serialized-as-json`.
...
- **R-CACHE-TTL-2.** Типовые значения по характеру данных:
  ...
  | OAuth/JWT keys | 5 min (default) | JWK Set (см. `auth-patterns/token-validated-by-library`) |
...
- **R-CACHE-TTL-X3.** Money-кеш **без TTL** или TTL > 1 минута без strict invalidation. См. `caching/money-data-needs-explicit-invalidation`.
...
- **R-CACHE-STAMP-3.** Для **hot keys** (top-products) — `Refresh-ahead` (см. `caching/cache-aside-is-default`) — фоновое обновление cache до expiry. Stampede исключён по дизайну.
...
|---|---|---|
| Кеш на write-методе | `caching/no-cache-on-write-path` | только read, write — `@CacheEvict` |
| Кеш доменного агрегата целиком | `caching/cache-projections-not-aggregates` | read-проекции (`OrderSummary`) |
| Money-кеш без TTL/invalidation | `caching/money-data-needs-explicit-invalidation`, `caching/money-data-needs-explicit-invalidation` | TTL ≤ 30s + явный evict |
| Кеш JWT/ABAC-валидации | `caching/no-caching-authorization-results` | JWK уже встроен (`auth-patterns/token-validated-by-library`); ABAC — каждый раз |
| `JdkSerializationRedisSerializer` | `caching/values-serialized-as-json` | `GenericJackson2JsonRedisSerializer` |
| `ConcurrentMapCacheManager` в multi-instance проде | `caching/distributed-cache-in-production` | RedisCacheManager |
| Один TTL на все кеши | `caching/explicit-ttl-per-cache` | per-cache config |
| `@EnableCaching` без CacheManager-бина | `caching/no-cache-without-manager` | явно сконфигурированный bean |
| Дефолтный keyGenerator на multi-arg методе | `caching/explicit-cache-key` | `key = "..."` SpEL явно |
| `Object.toString()` в ключе | `caching/explicit-cache-key` | bean ID или primitive |
| Один общий cache `shared-cache` | `caching/cache-name-is-namespace` | per-entity |
| PII / токены plain в ключе | `caching/no-sensitive-data-in-keys` | hash или ID-replacement |
| TTL = 0 / infinite | `caching/explicit-ttl-per-cache` | explicit duration |
| TTL > 24h для бизнес-данных | `caching/ttl-matches-data-nature` | разумные интервалы |
| `@CacheEvict(allEntries=true)` без причины | `caching/no-routine-full-flush` | по конкретному ключу |
| Полагаться только на TTL для money-consistency | `caching/ttl-is-not-consistency` | явный `@CacheEvict` на write |
| Eventual consistency без декларации | `caching/staleness-is-declared` | в OpenAPI `description` |
| Write-behind для money | `caching/cache-aside-is-default` | write-through или cache-aside |
| Mix паттернов на одном cache | `caching/cache-aside-is-default` | один cache = один паттерн |
| Игнорировать stampede на hot endpoints | `caching/stampede-protection` | distributed lock или refresh-ahead |
| `synchronized` для distributed-cache защиты | `caching/stampede-protection` | Redis lock |
| Отключение cache metrics | `caching/cache-metrics-enabled` | оставить включёнными |
