...
Свод правил применения CQRS (Command Query Responsibility Segregation) в Java/Spring-сервисах команды UCP. Каждое правило идентифицируется кодом (`cqrs/command-returns-minimum`, `cqrs/projection-has-no-logic-or-backflow`) — скилл `ucp-cqrs-review` цитирует эти коды в findings.

CQRS уже частично реализован в `backend/usecase-pattern/references/java/implementation.md` через маркеры `UseCaseCommand<R>` / `UseCaseQuery<R>` (правила `R-UC-*` и `R-CQRS-*` в Use Case Pattern). Этот гайд **углубляет** тему: когда CQRS оправдан, как разделять модели чтения и записи, что такое read-model и read-projection, как синхронизировать через события, антипаттерны.
...
- **R-CQRS-CMD-1.** Command — record, реализует `UseCaseCommand<R>` (см. `usecase-pattern/usecase-implements-marker`):
  ```java
...
- **R-CQRS-CMD-3.** Command handler:
  - `@Component` + `@Transactional` (RW по умолчанию, см. `jooq/transaction-on-handler`).
  ...
  - Записывает в outbox `<X>Event` (см. `kafka/publish-via-outbox`) — для синхронизации read-model.
...
- **R-CQRS-CMD-5.** Validation на command — частично контракт (через generated DTO + Jakarta, см. `validation/input-validated-at-edge`); бизнес-инварианты — в агрегате (`validation/domain-invariants-in-aggregate`).
...
- **R-CQRS-QRY-1.** Query — record, реализует `UseCaseQuery<R>` (см. `usecase-pattern/usecase-implements-marker`):
  ```java
...
- **R-CQRS-QRY-2.** Query handler:
  - `@Component` + `@Transactional(readOnly = true)` (см. `jooq/transaction-on-handler`).
  - Загружает данные через `<X>ViewRepository` (см. `jooq/view-repository-separate`) — отдельный интерфейс для read-проекций.
  - Возвращает read-DTO (`OrderSummary`), не агрегат.

- **R-CQRS-QRY-3.** Read-DTO — **read-model record** в `core/view/`:
  ```java
...
- **R-CQRS-SYNC-1.** **Синхронизация через outbox + Kafka** (см. `kafka/publish-via-outbox`):
...
- **R-CQRS-SYNC-2.** **Idempotent consumer** обязателен (см. `kafka/consumer-is-idempotent`). Read-model UPDATE может прийти дважды; consumer должен это распознавать (`processed_event` таблица или idempotent UPDATE через `version`-проверку).
...
- **R-CQRS-SYNC-X3.** **Schema-coupled events.** Если event payload — это generated POJO write-схемы, любой ALTER TABLE на write-side ломает consumers. См. `kafka/event-schema-forward-compatible` (event versioning).
...
- **R-CQRS-TIER-2.** **Уровень 2** (Use Case Pattern): lightweight CQRS: маркеры `UseCaseCommand`/`UseCaseQuery` обязательны на каждой операции, голого `UseCase` нет. Read и write через **один и тот же `<X>Repository`**, но read-методы через `SelectMode.NO_LOCK` + `@Transactional(readOnly = true)`.

- **R-CQRS-TIER-3.** **Уровень 3** (DDD + Hexagonal): полный split. Появляется `<X>ViewRepository` (см. `jooq/view-repository-separate`) с read-DTO; write — через `<X>Repository` с агрегатом и `FOR UPDATE`.
...
|---|---|---|
| Полный CQRS «just in case» для нового сервиса | `cqrs/lightweight-first-full-on-evidence` | lightweight маркеры на старте |
| Разделение баз без явной причины | `cqrs/lightweight-first-full-on-evidence` | conscientious decision |
| Command-handler делает отдельный SELECT | `cqrs/command-handler-does-not-query` | load-aggregate one-shot |
| Command возвращает полный read-DTO | `cqrs/command-returns-minimum` | минимум: id/статус |
| Несколько агрегатов в одном command | `cqrs/command-changes-one-aggregate` | saga (см. `R-DIST-SAGA-*`) |
| Query handler делает write | `cqrs/query-is-read-only` | перенести в command |
| Query грузит агрегат целиком и маппит | `cqrs/read-via-projection-not-aggregate` | `<X>ViewRepository` с минимум данных |
| Query возвращает агрегат наружу | `cqrs/query-returns-read-model` | read-DTO |
| Read-model с бизнес-логикой | `cqrs/projection-has-no-logic-or-backflow` | проекция, без триггеров логики |
| Read-model как source-of-truth | `cqrs/read-model-is-rebuildable` | восстановима из write-side |
| Bidirectional sync | `cqrs/projection-has-no-logic-or-backflow` | one-way: write → events → read |
| Sync read-model в command-tx | `cqrs/read-model-synced-by-events` | outbox + Kafka |
| Sync через PG triggers | `cqrs/read-model-synced-by-events` | outbox + consumer |
| Schema-coupled events | `cqrs/events-not-coupled-to-write-schema` | versioned event-records |
| Уровень 1 с CQRS-маркерами без enforcement | `cqrs/split-matches-maturity-level` | без маркеров |
| Event-driven read-model с одним repository для R+W | `cqrs/split-matches-maturity-level` | отдельный ViewRepository |
