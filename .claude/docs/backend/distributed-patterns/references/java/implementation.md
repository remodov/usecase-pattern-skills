# Distributed Patterns — реализация

Свод правил применения распределённых паттернов (saga, eventual consistency, idempotency, distributed transactions, two-phase alternatives) в Java/Spring-сервисах команды UCP. Каждое правило идентифицируется кодом (`distributed/orchestration-versus-choreography`, `distributed/receiver-deduplicates`) — скилл `ucp-distributed-review` цитирует эти коды в findings.

Гайд опирается на ранее покрытые темы: outbox через `R-KFK-OBX-*` и `pg-runtime/skip-locked-for-queues`, idempotent consumer `R-KFK-IDEM-*`, retry topics `R-KFK-RTRY-*`, CQRS sync `R-CQRS-SYNC-*`, AUTH-19 (Idempotency-Key для money). Этот гайд **связывает** их в единый набор правил для cross-service flows.
...
- `auth-patterns/money-commands-need-idempotency-key` (`auth-patterns/money-commands-need-idempotency-key`) — Idempotency-Key для money.
- `R-RES-*` — CB/Bulkhead/Retry для outbound.
- `pg-runtime/pessimistic-versus-optimistic` (`pg-runtime/pessimistic-versus-optimistic`, `L`) (PG runtime) — optimistic locking через version.
- `pg-runtime/advisory-lock-for-singleton` (`pg-runtime/advisory-lock-for-singleton`, `L`) — advisory locks.
...
- **R-DIST-WHEN-2.** Если операция **в одном сервисе и одном PG** — не нужны распределённые паттерны. Используй обычный `@Transactional` (`jooq/transaction-on-handler`) и атомарность БД.
...
- **R-DIST-IDEM-1.** **Каждое cross-service сообщение** имеет уникальный ID:
  - Kafka events — `eventId UUID v7` (см. `kafka/event-payload-hygiene`).
  - HTTP money-команды — `Idempotency-Key` header (см. `auth-patterns/money-commands-need-idempotency-key`).
  - Saga steps — `sagaId + stepName` уникальный ключ.

- **R-DIST-IDEM-2.** **Receiver хранит processed-events** в БД (`processed_event` таблица, см. `kafka/processed-record-in-same-transaction`). Перед обработкой — проверка наличия. После — запись в той же транзакции.
...
- **R-DIST-IDEM-X2.** **Полагаться на receiver-side только**. Sender (producer) тоже должен иметь exactly-once гарантии (Kafka `enable.idempotence: true`, см. `kafka/producer-is-idempotent`).
...
- **R-DIST-OBX-2.** **Inbox pattern** для входящих сообщений (опционально, для critical-сценариев):
  ...
  - Большинство случаев — inbox избыточен, достаточно `processed_event` dedup-таблицы (см. `kafka/processed-record-in-same-transaction`).
...
- **R-DIST-OBX-X1.** **Direct send из command-handler** без outbox. См. `kafka/publish-via-outbox`.

- **R-DIST-OBX-X2.** **`@TransactionalEventListener`** для отправки в Kafka. См. `kafka/publish-via-outbox`.
...
- **R-DIST-COMP-X1.** **Saga без compensation** (см. `distributed/every-step-has-compensation`).
...
|---|---|---|
| Распределённые паттерны для one-сервиса | `distributed/patterns-only-when-crossing-services` | `@Transactional` |
| Микросервисы из амбиций | `distributed/patterns-only-when-crossing-services` | modular monolith |
| 2PC / JTA вместо saga | `distributed/no-two-phase-commit`, `distributed/no-two-phase-commit` | saga с compensation |
| Saga без compensation | `distributed/every-step-has-compensation`, `distributed/every-step-has-compensation` | каждый шаг — compensation |
| Saga state in-memory | `distributed/saga-state-is-persistent` | `saga_<name>` таблица |
| Saga смешана с use case | `distributed/saga-separate-from-use-cases` | отдельный orchestrator |
| Receiver без dedup для money | `distributed/receiver-deduplicates` | processed_event + Idempotency-Key |
| Только receiver-side dedup | `distributed/money-double-protection` | producer enable.idempotence + receiver dedup |
| Idempotency-Key = новый UUID каждый раз | `distributed/idempotency-key-per-operation` | один ключ на бизнес-операцию |
| Молчаливая eventual consistency | `distributed/bounded-and-declared-staleness` | в OpenAPI description |
| 2PC для immediate consistency | `distributed-patterns/no-two-phase-commit`, `distributed/no-two-phase-commit` | redesign boundary либо EC |
| Direct send из command без outbox | `distributed/outbox-for-outgoing-events` | outbox pattern |
| @TransactionalEventListener для Kafka | `distributed/outbox-for-outgoing-events` | outbox-relay |
| DELETE как compensation | `distributed/compensation-is-semantic-and-idempotent` | semantic state-change (`cancelled`) |
| Compensation без DLQ при failure | `distributed/failed-compensation-goes-to-review` | DLQ + manual review |
| ChainedTransactionManager multi-DS | `distributed/no-two-phase-commit` | один PG или saga |
