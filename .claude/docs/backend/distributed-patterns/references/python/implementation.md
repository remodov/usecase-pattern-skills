# Distributed Patterns — реализация на Python (saga + UoW + aiokafka)

Реализация язык-нейтрального контракта `../spec.md` (`R-DIST-*`) на Python. Паттерны
...
один сервис + один PG → обычный UoW + атомарность БД, без распределённых паттернов. `distributed/patterns-only-when-crossing-services` — сначала
проверь альтернативы (объединить BC, modular monolith). `distributed/patterns-only-when-crossing-services` — saga для двух операций в одной БД.
`distributed/patterns-only-when-crossing-services` — микросервисы «из амбиций» (latency, debugging, новые failure modes) — лучше modular monolith.
...
(события без координатора) для простых (2–3 шага). `distributed/saga-state-is-persistent` — **saga state в БД** (`saga_<name>`-таблица:
`saga_id`, `status`, `current_step`, `payload`) — видимость, recovery, audit. `distributed/saga-state-is-persistent` — `saga_id` сквозной
в каждом сообщении.
...
`distributed/no-two-phase-commit` — 2PC/XA через distributed-TX вместо saga (Kafka не XA). `distributed/every-step-has-compensation` — saga без compensation
(«полусделанная» транзакция). `distributed/saga-state-is-persistent` — saga state in-memory (рестарт теряет in-flight saga). `distributed/saga-separate-from-use-cases` —
...
`distributed/message-has-unique-id` — у каждого cross-service сообщения уникальный id (`event_id`/`message_id`). `distributed/receiver-deduplicates` —
receiver хранит `processed_event` (проверка до, запись в той же UoW-транзакции, cross-ref `kafka/processed-record-in-same-transaction`).
`distributed/http-idempotency-record` — для HTTP-команд хранить `(idempotency_key, response)`; повтор с тем же ключом → сохранённый ответ;
другой `command_hash` с тем же ключом → `409 Conflict`. `distributed/money-double-protection` — money: `Idempotency-Key` +
UNIQUE `(provider_id, external_payment_id)`. `distributed/http-idempotency-record` — TTL idempotency-записей 24–72ч.
...
exactly-once: aiokafka `enable_idempotence=True`, `kafka/producer-is-idempotent`). `distributed/idempotency-key-per-operation` — `Idempotency-Key` = новый UUID
...
`distributed/bounded-and-declared-staleness` — декларация в OpenAPI (задержка read-проекции) — FastAPI: в описании эндпоинта/response-модели.
`distributed/read-your-writes-when-required` — read-your-writes при необходимости (читать из write-side / version-токен). `distributed/bounded-and-declared-staleness` — bounded
staleness с явным SLO + alert. `distributed/ordering-by-version` — causal consistency через `version`-поля (receiver применяет, только
...
`distributed/outbox-for-outgoing-events` — **outbox** для исходящих событий обязателен (`R-KFK-OBX-*`, `ucp-py-kafka-*`). `distributed/inbox-for-critical-messages` —
...
публикация после commit без outbox (`kafka/publish-via-outbox`).

## 6. Compensation (`R-DIST-COMP-*`)

`distributed/every-step-has-compensation` — у каждой command в саге есть compensation-команда. `distributed/compensation-is-semantic-and-idempotent` — compensation идемпотентен
(saga может повторить). `distributed/compensation-is-semantic-and-idempotent` — **semantic compensation**, не технический rollback (был платёж →
compensation = refund новой транзакцией). `distributed/compensation-is-semantic-and-idempotent` — compensation оставляет audit trail (статус `refunded`
+ ссылка на оригинал), не DELETE/UPDATE-с-потерей.

`distributed/every-step-has-compensation` — saga без compensation. `distributed/compensation-is-semantic-and-idempotent` — `DELETE` как compensation («создан заказ» → «cancelled»,
не `DELETE FROM orders`). `distributed/failed-compensation-goes-to-review` — compensation, которое может упасть без повторного compensation/DLQ
(висящие деньги).
...
`distributed/no-two-phase-commit` — 2PC/XA в стеке (Kafka не XA, не масштабируется, SPOF). `distributed/no-two-phase-commit` — единая распределённая
транзакция через несколько datasource. `distributed/no-two-phase-commit` — «цепочка» commit'ов по нескольким сессиям/БД (best-effort,
...
event-driven sync. `distributed/patterns-only-when-crossing-services` — modular monolith (несколько BC в одном процессе с одним PG) при tight coupling —
