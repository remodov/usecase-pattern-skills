# Graceful Shutdown — реализация на Go (net/http + chi)

Реализация контракта `../spec.md` (`R-SHUT-*`) на Go-стеке (net/http + chi). Коды правил —
...
`graceful-shutdown/web-server-graceful-enabled` — `http.Server.Shutdown(ctx)` вместо `http.Server.Close()`: Shutdown дожидается in-flight
запросов; Close рвёт их немедленно.

`graceful-shutdown/shutdown-budget-is-explicit` — таймаут фазы shutdown задаётся явным контекстом (20–30s; в 60s budget после preStop остаётся
место для Kafka/БД):
...
`graceful-shutdown/readiness-off-first` — readiness-флаг переводится в `false` первым, до `srv.Shutdown`. Health-endpoint проверяет
...
`graceful-shutdown/readiness-off-first` — раздельные `/health/live` и `/health/ready` — chi-маршруты с разной логикой:
...
`graceful-shutdown/readiness-off-first` ❌ `var shuttingDown bool` как обычная переменная без `atomic` и без связи с health-эндпоинтом —
...
`graceful-shutdown/web-server-graceful-enabled` — `http.Server.Shutdown` дожидается всех in-flight запросов; новые соединения не принимаются
автоматически. Дополнительной логики не требуется — механизм встроен.

`graceful-shutdown/prestop-delay` — `preStop` hook со `sleep 10` обязателен в Deployment:
...
`graceful-shutdown/long-operations-are-async` — долгие эндпоинты (>10s) — `202 Accepted` + polling или постановка задачи в очередь:
...
`graceful-shutdown/web-server-graceful-enabled` ❌ `srv.Close()` вместо `srv.Shutdown(ctx)` — немедленно рвёт активные соединения, аннулирует graceful.

---

## 3. Kafka shutdown (`R-SHUT-KFK-*`)

`graceful-shutdown/consumer-finishes-batch` — consumer (`segmentio/kafka-go`) дожимает текущий batch и коммитит offset перед остановкой;
...
`graceful-shutdown/consumer-finishes-batch` — listener не запускает долгий каскад (HTTP с retry на 30s не укладывается в бюджет) — каскад в
async-flow или outbox.

`graceful-shutdown/consumer-finishes-batch` — ручной `CommitMessages` после обработки каждого сообщения (или batch); `kafka-go` не имеет
авто-коммита в смысле kafka-go reader-API — нужно явно вызывать `CommitMessages`.

`graceful-shutdown/consumer-finishes-batch` — flush и закрытие writer'а на shutdown:
...
`graceful-shutdown/consumer-finishes-batch` ❌ `AutoCommit` на стороне kafka-go reader с `CommitInterval` — часть offset фиксируется до
...
`graceful-shutdown/connection-pool-closes-last` — пул `pgx` (`pgxpool.Pool`) закрывается последним, после завершения HTTP/задач/consumer'а;
...
`graceful-shutdown/transactions-finish-in-own-channel` — активные транзакции завершаются через свой канал (HTTP — via Shutdown, scheduler/consumer —
через отмену контекста с дожатием `WaitGroup`).

`graceful-shutdown/transactions-finish-in-own-channel` — миграции (`golang-migrate` или аналог) запускаются только на старте, не на shutdown.

`graceful-shutdown/connection-pool-closes-last` ❌ `pool.Close()` раньше завершения фоновых задач — pgx паникует при попытке взять соединение из
закрытого пула.
...
`graceful-shutdown/background-tasks-finish-iteration` — фоновые горутины завершают текущую итерацию, не начинают новую; управляются через
...
`graceful-shutdown/background-tasks-finish-iteration` — горутина с долгим каскадом держит `WaitGroup` до конца критичной секции; отмена контекста —
...
`graceful-shutdown/outbox-relay-checks-readiness` — outbox-relay завершает текущий batch (`FOR UPDATE SKIP LOCKED`), не начинает новый; цикл
...
`graceful-shutdown/background-tasks-finish-iteration` ❌ Отмена горутины без `WaitGroup.Wait()` — задача убита в середине, частичные изменения без
...
`graceful-shutdown/shutdown-budget-is-explicit` — `terminationGracePeriodSeconds: 60` явно в Deployment; preStop sleep — отдельный бюджет сверху.

`graceful-shutdown/readiness-off-first` — `readinessProbe` → `/health/ready`, `livenessProbe` → `/health/live`; на shutdown
readiness=503 (liveness-падение рестартит pod).

`graceful-shutdown/rolling-update-keeps-capacity` — `maxSurge: 1, maxUnavailable: 0` (нулевой downtime при rolling update).
...
`graceful-shutdown/prestop-delay` ❌ Отсутствие preStop — 5–15s после SIGTERM kube-proxy ещё льёт трафик → 502.

`graceful-shutdown/shutdown-budget-is-explicit` ❌ `terminationGracePeriodSeconds: 30` (default) при 25s graceful — preStop не помещается,
...
`graceful-shutdown/in-flight-operations-are-retry-safe` — операции, которые SIGTERM может прервать, retry-safe: write с `Idempotency-Key` в заголовке,
...
`graceful-shutdown/in-flight-operations-are-retry-safe` ❌ Money-операция (списание с Customer) без `Idempotency-Key` под retry — SIGTERM в момент
retry → новый pod спишет повторно (запрещено `resilience/retry-only-when-safe`).

---

## 8. Бюджеты и observability (`R-SHUT-OBS-*`)

`graceful-shutdown/shutdown-budget-is-explicit` — реалистичный cumulative-бюджет: preStop 10s + `http.Server.Shutdown` ≤25s + scheduler/async
≤20s + kafka consumer ≤15s ≤ 60s; не помещается — сократить batch (100→20), не увеличивать budget.

`graceful-shutdown/shutdown-is-observable` — метрика `app_shutdown_duration_seconds` + структурный лог начала/конца shutdown:
...
`graceful-shutdown/shutdown-is-observable` — лог факта SIGTERM первым:
...
`graceful-shutdown/shutdown-is-observable` ❌ `slog.Error` при нормальном закрытии пула/consumer'а — шум в alert-канале на каждый деплой;
