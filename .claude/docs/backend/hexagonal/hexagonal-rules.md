# Hexagonal Architecture — индекс правил (язык-нейтральный)
...
> **Реализация по языкам** — `java/hexagonal-style-guide.md` (multi-module Gradle + Spring + ArchUnit) и
> `python/hexagonal-style-guide.md` (пакеты core/adapters/app + import-linter); открывай нужный точечно.
> Коды: `R-HEX-<GROUP>-<N>` — обязательно, `R-HEX-<GROUP>-X<N>` — запрещено. **Коды общие для всех языков** —
> меняется механизм изоляции (compile-time Gradle-модули в Java ↔ import-linter-контракт по пакетам в Python).

## 1. Когда переходить на Hexagonal
**MUST:**
- **R-HEX-WHEN-1.** Hexagonal — часть **Уровня 3** (DDD + Hexagonal: агрегаты, ports/adapters, архитектурный тест (import-контроль)). На Уровне 1–2 — overkill.
...
## 2. Структура модулей
**MUST:**
- **R-HEX-MOD-1.** Многомодульный проект:
- **R-HEX-MOD-2.** **`core/` — единственный модуль без фреймворка**. Чистый язык + DDD-библиотеки (`ddd-building-blocks`, `usecase-pattern`). Это даёт:
- **R-HEX-MOD-3.** **Каждый out-adapter** — отдельный модуль сборки:
- **R-HEX-MOD-4.** **Каждый in-adapter** — отдельный модуль сборки:
- **R-HEX-MOD-5.** **`bootstrap/`** — composition root: entrypoint композиции, DI-конфиг для wiring beans, конфиг, `Dockerfile`. Зависит от `core/` + всех адаптеров. Никто не зависит от `bootstrap/`.
**MUST NOT:**
- **R-HEX-MOD-X1.** **Один модуль сборки** для всего сервиса с папками `core/`, `adapter/`. Compile-time нарушения не ловятся — кто-то добавит импорт фреймворка в `core/` package и никто не заметит.
- **R-HEX-MOD-X2.** **`core/` зависит от `persistence/`** (или любого адаптера) в манифест модуля. Стрелка зависимостей всегда: `bootstrap/ → core/ ← adapters`. Не наоборот.
...
- **R-HEX-CORE-3.** **DI-аннотации на классах core/** — **разрешены** только если использовать стартер DI-стартер, который автоматически их пикает. В этом случае фреймворк **не компилирует** core, а просто сканирует beans из uber-jar. Альтернатива — голые POJO + явные DI-фабрики в `bootstrap/`.
...
**MUST NOT:**
- **R-HEX-CORE-X1.** **фреймворк-импорт в `core/`** (импорт фреймворка). Compile-time guard через архитектурный тест (import-контроль) — обязательный архитектурный тест (см. `R-HEX-TEST-*`).
- **R-HEX-CORE-X2.** **импорт ORM/SQL в `core/`** (`import org.jooq.*`). ORM/SQL — persistence-деталь, живёт в `persistence/`-модуле. Domain работает с domain-объектами; mapping POJO ↔ Domain — в `persistence/<X>DomainRecordMapper`.
...
- **R-HEX-CORE-X4.** **Generated POJO / Record (ORM/SQL) в `core/`** как доменный тип. POJO — internal деталь persistence; в core используется domain entity.
- **R-HEX-CORE-X5.** **HTTP / REST DTO в `core/`** (REST request/response DTO). REST DTO — деталь in-adapter; в core — UseCase/Command/domain entity.
...
**MUST NOT:**
- **R-HEX-PORT-X1.** **Port в out-adapter** (порт-файл в `<system>-out-adapter/`). Port — контракт core-к-инфраструктуре, живёт в `core/`. Adapter — реализация.
...
- **R-HEX-AIN-2.** **Controller** реализует сгенерированный API-интерфейс (см. `R-OAS-1`), маппит request DTO в `UseCase` command, dispatchит:
...
- **R-HEX-AIN-4.** **In-adapter знает web-фреймворк + REST** (HTTP, JSON, валидация), **не знает** про другие адаптеры (`persistence/`, `<system>-out-adapter/`).
**MUST NOT:**
...
- **R-HEX-AIN-X3.** **Controller возвращает domain entity** наружу как HTTP-response. Используй REST-DTO (сгенерированный из OpenAPI).
...
- **R-HEX-AOUT-4.** **Out-adapter знает свою инфраструктуру**: `persistence/` знает ORM/SQL; `sber-out-adapter/` знает свой HTTP-клиент + DTO системы; `kafka-out-adapter/` знает Kafka. Между собой adapter'ы **не знают**.
**MUST NOT:**
...
- **R-HEX-BOOT-3.** **entrypoint композиции** или дефолтный component scan покрывает все адаптеры. Альтернатива — explicit импорт модулей через DI-конфиг.
**MUST NOT:**
- **R-HEX-BOOT-X1.** **`bootstrap/` содержит бизнес-логику** или REST-контроллеры. Только композиция и configs.
- **R-HEX-BOOT-X2.** **entrypoint композиции в `core/` или `*-adapter/`**. Только в `bootstrap/` — иначе невозможно собрать сервис из частей.

## 8. Архитектурные тесты
**MUST:**
- **R-HEX-TEST-1.** **архитектурный тест (import-контроль)-тесты в `bootstrap/src/test/java/`** (или отдельный модуль `architecture-tests/`) проверяют:
...
- **R-HEX-TEST-3.** **единый scan корневого пакета** для всех тестов — единая точка скана.
**MUST NOT:**
