# Hexagonal Architecture — реализация на Python (пакеты + import-linter)

Реализация язык-нейтрального контракта `../spec.md` (`R-HEX-*`) на Python. Коды общие с Java; меняется
...
`hexagonal/level-three-only` — Hexagonal = Уровень 3 (DDD + ports/adapters + import-linter). Уровень 1–2 — overkill: плоский
`app/` без слоёв. `hexagonal/level-three-only` — cargo-cult (сервис из 3 эндпоинтов в полной раскладке) — ceremony.
`hexagonal/no-partial-adoption` — частичный Hexagonal (есть `core/`, но роутеры мешают бизнес-логику с HTTP) — либо полностью, либо никак.

## 2. Структура (`R-HEX-MOD-*`)

Вместо gradle-модулей — пакеты с контрактом import-linter. `hexagonal/module-per-part`/`hexagonal/core-free-of-framework` — `core/` не импортирует
...
`hexagonal/module-per-part` — отсутствие import-linter-контракта (полагаться на дисциплину) — кто-нибудь импортнёт SQLAlchemy в
`core/` и никто не заметит. `hexagonal/core-free-of-framework` — `core/` импортирует `adapters/*` — стрелка всегда `app → adapters → core`.
`hexagonal/in-adapter-per-audience` — user- и admin-роутеры в одном модуле без разделения — теряется изоляция security (отдельные пакеты
`adapters/in/http/{user,admin}/` + contract).

## 3. Core (`R-HEX-CORE-*`)

`hexagonal/core-free-of-framework` — `core/` зависит только от stdlib + доменных типов. `hexagonal/rich-domain-model` — rich domain: логика в
агрегате (`order.confirm()`), не в `*Service` (cross-ref `ddd-tactical/aggregate-root-is-single-entry`, `ucp-py-ddd-tactical-*`).

`hexagonal/core-free-of-framework` — FastAPI-импорт в `core/` (enforce import-linter). `hexagonal/core-free-of-framework` — SQLAlchemy-импорт в `core/`
(ORM — деталь persistence; маппинг в `adapters/out/persistence/<x>_mapper.py`, cross-ref `sqlalchemy/explicit-mapper`).
`hexagonal/rich-domain-model` — анемичная модель. `hexagonal/no-generated-types-in-core` — ORM-модель как доменный тип в `core/`. `hexagonal/no-generated-types-in-core` —
...
`hexagonal/outbound-port-interface-in-core` — outbound-порт = `Protocol` в `core/<bc>/port/out/`, описывает что нужно core (cross-ref
`ddd-tactical/repository-port-in-domain`, `hexagonal/outbound-port-interface-in-core`). `hexagonal/port-speaks-domain-types` — методы порта оперируют domain-типами, не DTO внешней системы.
`hexagonal/port-exceptions-in-core` — port-исключения объявлены в `core/` (`PaymentPortError`); подклассы (`SberError`) — в out-adapter;
handler ловит базовый. `hexagonal/inbound-port-is-use-case` — inbound-порт = UseCase + Handler (вход через `Dispatcher`), отдельный
...
сигнатуре порта (адаптер мапит внутри). `hexagonal/absence-is-not-error` — `X | None` из порта, где отсутствие = ошибка (брось
доменное `OrderNotFoundError`). `hexagonal/outbound-port-interface-in-core` — порт как класс, не `Protocol`/ABC — убивает подмену в тестах.
...
`hexagonal/in-adapter-per-audience` — пакет `adapters/in/http/` на HTTP-вход (+ отдельные пакеты на другие типы входа: kafka-consumer).
`hexagonal/controller-dispatches-only`/`hexagonal/rest-mapping-in-adapter` — роутер маппит Pydantic request-DTO → UseCase command, зовёт `Dispatcher`; отдельный
маппер (`order_request_mapper.py`), не возвращай domain-агрегат как HTTP-ответ. `hexagonal/adapters-do-not-know-each-other` — in-adapter знает
FastAPI/Pydantic, не знает про `adapters/out/*`.

`hexagonal/controller-dispatches-only` — бизнес-логика в роутере (`if req.amount > 100`). `hexagonal/controller-dispatches-only` — роутер зовёт репозиторий
напрямую (только через `Dispatcher` → Handler, cross-ref `usecase-pattern/entry-calls-dispatcher`). `hexagonal/rest-mapping-in-adapter` — роутер возвращает domain-агрегат
наружу (маппи в response-DTO). `hexagonal/adapters-do-not-know-each-other` — `adapters/in/*` импортирует `adapters/out/*` — адаптеры зависят от
...
`hexagonal/out-adapter-per-system` — пакет `adapters/out/<system>/` на каждую внешнюю систему (per-system isolation, cross-ref
`resilience/client-per-external-system`). `hexagonal/out-adapter-per-system` — адаптер реализует порт-`Protocol` из `core/`. `hexagonal/adapter-maps-not-decides` — маппер domain ↔ DTO
внешней системы в адаптере. `hexagonal/adapters-do-not-know-each-other` — адаптер знает свою инфраструктуру (`persistence/` — SQLAlchemy;
...
бизнес-логика в out-adapter (`if sber_response.code == 1`); адаптер мапит, решает handler. `hexagonal/out-adapter-per-system` — один
адаптер реализует порты разных доменов. `hexagonal/adapters-do-not-know-each-other` — out-adapter инжектит другой out-adapter (координация двух —
...
`hexagonal/bootstrap-composition-only`/`hexagonal/composition-covers-all-adapters` — `app/` = composition root: `create_app()` factory, `container` (DI-wiring портов на
адаптеры), `lifespan`, `settings`, `Dockerfile` (cross-ref `PYBOOT-5/8`). Зависит от `core/` + всех адаптеров; от
`app/` не зависит никто.

`hexagonal/bootstrap-composition-only` — бизнес-логика/роутеры в `app/` (только композиция и конфиг). `hexagonal/bootstrap-composition-only` — создание FastAPI-app
...
`hexagonal/architecture-tests-required`/`hexagonal/architecture-test-required-check` — `import-linter` (контракт layers + при необходимости forbidden/independence) запускается
в CI как required check; PR не мерджится при падении. Это аналог ArchUnit — compile-time-подобный guard на
импортах. `hexagonal/single-scan-root` — единый `root_package` в контракте.

`hexagonal/architecture-tests-required` — только code-review для enforcement границ — человек пропустит импорт; нужен `lint-imports` в CI.
