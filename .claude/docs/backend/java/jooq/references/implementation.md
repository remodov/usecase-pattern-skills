...
Свод правил использования jOOQ в Java-сервисах команды UCP. Каждое правило идентифицируется кодом (`jooq/codegen-from-live-schema`, `jooq/type-safe-query-building` и т. п.) — скиллы `ucp-jooq-design` и `ucp-jooq-review` цитируют эти коды в findings.

Гайд опирается на:
- `spring-bootstrap/single-persistence-mechanism`/`spring-bootstrap/prefer-generated-types`/`spring-bootstrap/prefer-generated-types`/`spring-bootstrap/external-dtos-are-handcrafted` (см. `backend/java/spring-bootstrap/references/implementation.md`) — почему jOOQ и только generated классы.
- `pg-runtime/row-lock-inside-transaction`/`pg-runtime/row-lock-inside-transaction` (см. `backend/pg-runtime/references/implementation.md`) — locking-синтаксис и обязательность `@Transactional`.
...
- **R-JOOQ-CFG-4.** TIMESTAMP/TIMESTAMPTZ-колонки маппятся на `java.time.OffsetDateTime` через `forcedTypes`. `LocalDateTime` запрещён для persistent-полей (см. `jooq/codegen-settings`).
...
- **R-JOOQ-CFG-6.** Forced types для domain-enum'ов настраиваются через `<enumConverter>true</enumConverter>` если java enum уже существует в `core/`. Иначе jOOQ сам генерирует enum (см. `spring-bootstrap/prefer-generated-types`).
  ```xml
  <forcedType>
...
- **R-JOOQ-CFG-X1.** `setDaos(true)` — generated DAO в репозитории не используются (см. `jooq/no-generated-daos`). Генерация лишних классов.
...
- **R-JOOQ-REPO-1.** Domain-репозиторий — interface в `core/port/out/repository/`. jOOQ-имплементация — в модуле `persistence/`, имя класса `Jooq<X>Repository implements <X>Repository`.
  ```java
  // core/port/out/repository/OrderRepository.java
...
- **R-JOOQ-REPO-X1.** Использование jOOQ Generated DAO (`*Dao`) — запрещено `spring-bootstrap/prefer-generated-types` и `jooq/no-generated-daos`. Вместо них пишутся свои репозитории.
...
- **R-JOOQ-REPO-X3.** Spring Data JDBC, JPA, Hibernate, MyBatis, JdbcTemplate в любой форме — нарушение `spring-bootstrap/single-persistence-mechanism`.
...
- **R-JOOQ-FLT-1.** Сложные WHERE-условия выносятся в `<X>FilterConditionBuilder` — Spring `@Component`, не статическая утилита (нужны зависимости — `DSLContext` для подзапросов).
  ...
  ```
- **R-JOOQ-FLT-2.** `<X>Filter` — record/POJO в `core/port/out/filter/`, иммутабельный. Поля nullable, чтобы фильтр игнорировался если значение не задано.
...
- **R-JOOQ-MAP-3.** Enum-перевод — через generated jOOQ enum + domain enum:
  ...
  ```
  Или через `forcedType` с `<enumConverter>true</enumConverter>` — тогда конвертация автоматическая (см. `jooq/enums-and-documents-mapping`).
- **R-JOOQ-MAP-4.** JSONB-колонки — через `JooqJsonbHelper` (Spring-bean с инжектируемым `ObjectMapper`):
  ...
  ```
- **R-JOOQ-MAP-5.** Timestamp-конверсия не делается в маппере вручную — `forcedType OffsetDateTime` (см. `jooq/codegen-settings`) делает это в codegen-time.
...
- **R-JOOQ-PAG-1.** Domain-репозиторий возвращает `PaginationView<T>` — record/POJO в `core/view/`:
  ```java
...
- **R-JOOQ-PAG-2.** Offset-based пагинация в репозитории:
  ...
  - Контракт API — `rest-api/pagination-forms` (page 1-based в публичном REST). В репозитории — 0-based, конвертируется на handler-уровне.
...
- **R-JOOQ-PAG-4.** Cursor — непрозрачный токен на уровне API (см. `rest-api/pagination-forms` в REST guide). На уровне репозитория cursor — record `(createdAt, id)` или подобный composite-key.
...
- **R-JOOQ-LCK-1.** В `core/port/out/SelectMode.java` — domain-enum:
  ```java
...
- **R-JOOQ-LCK-2.** `applyLock(query, mode)` — private helper в репозитории:
  ...
  ```
- **R-JOOQ-LCK-3.** Любой `forUpdate()`/`skipLocked()`-запрос — внутри `@Transactional`-метода (см. `pg-runtime/row-lock-inside-transaction`). Иначе jOOQ откроет/закроет соединение, лок отпустится мгновенно.
- **R-JOOQ-LCK-4.** Для optimistic locking используется `version`-колонка в схеме (см. `pg-runtime/pessimistic-versus-optimistic`), а не `withExecuteWithOptimisticLocking(true)` в `Settings`. Причина: `version`-колонка явная и видима всем (миграциям, debug, ручным правкам), `Settings` — скрытая магия.
...
- **R-JOOQ-TX-X2.** `@Transactional` на сервисном слое в обход handler'ов. У UCP — handler единственная точка транзакции (см. `backend/usecase-pattern/references/java/implementation.md` § Handlers).
...
- **R-JOOQ-VIEW-3.** Read-DTO — record в `core/view/`. Иммутабельный, без бизнес-логики.
...
|---|---|---|
| Generated DAO в репозитории | `jooq/no-generated-daos`, `jooq/no-generated-daos` | `Jooq<X>Repository implements <X>Repository` |
| Plain SQL `dslContext.fetch("...")` | `jooq/type-safe-query-building` | DSL: `selectFrom(...)` |
| Конкатенация в condition | `jooq/type-safe-query-building` | bind через `.eq(value)` |
| `record.set...().store()` для UPDATE | `jooq/fetch-and-update-forms` | `update(table).set(...)` |
| `.fetchOne()` где допустим null | `jooq/absence-and-existence-are-explicit` | `.fetchOptional()` |
| `.fetchCount() > 0` | `jooq/absence-and-existence-are-explicit` | `.fetchExists()` |
| Magic-string alias в multiset | `jooq/nested-collections-in-one-query` | константа из `SelectMultisetAliasKeys` |
| Lazy-fetch nested через отдельный запрос | `jooq/nested-collections-in-one-query` | `multiset()` или batch-fetch |
| Inline ветвящийся WHERE | `jooq/filters-extracted-to-builder` | `<X>FilterConditionBuilder` + `andIfNotNull` |
| POJO/Record возвращается наружу | `jooq/repository-speaks-domain-types` | mapper → domain |
| `MapStruct` с assemble-логикой | `jooq/mapper-is-explicit-class` | plain Java mapper |
| `DSL.using(...)` вручную | `jooq/dsl-context-is-singleton-bean` | Spring-bean `DSLContext` |
| `LocalDateTime` для timestamptz | `jooq/codegen-settings` | `OffsetDateTime` |
| `setImmutablePojos(true)` | `jooq/no-generated-daos` | мутабельные POJO |
| Codegen из xml/ddl | `jooq/codegen-from-live-schema` | Liquibase + Testcontainers + introspection |
| `forUpdate()` без `@Transactional` | `jooq/locking-select-inside-transaction`, `pg-runtime/row-lock-inside-transaction` | `@Transactional` на handler'е |
| Глобальный optimistic-locking через `Settings` | `jooq/optimistic-locking-via-version-column` | `version`-колонка, `pg-runtime/pessimistic-versus-optimistic` |
| `@Transactional` на репозитории | `jooq/transaction-on-handler` | на handler'е |
| Вложенные read-методы в основном `<X>Repository` | `jooq/view-repository-separate` | отдельный `<X>ViewRepository` |
| `fetchAll()` без LIMIT в UI-эндпоинте | `jooq/pagination-returns-domain-view` | `LIMIT/OFFSET` или cursor |
| JdbcTemplate / JPA / MyBatis | `jooq/no-other-persistence-frameworks`, `spring-bootstrap/single-persistence-mechanism` | jOOQ |

Финальная сводка: правил «Обязательно» — около 50, «Запрещено» — около 25. Findings ревью цитируют конкретный код (`jooq/nested-collections-in-one-query`, `jooq/locking-select-inside-transaction`).
