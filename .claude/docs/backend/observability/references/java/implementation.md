# Observability — реализация

Свод правил наблюдаемости в Java/Spring-сервисах команды UCP: structured logging с MDC, Micrometer-метрики (RED/USE), OpenTelemetry tracing, Actuator health-checks, propagation context через async, SLO + alerts. Каждое правило идентифицируется кодом (`observability/structured-logs-in-production`, `observability/low-cardinality-labels`) — скилл `ucp-observability-review` цитирует эти коды в findings.
...
- `auth-patterns/no-pii-in-logs-and-events` (`auth-patterns/no-pii-in-logs-and-events`) (Auth) — PII в логах **запрещено** (главное правило observability ↔ security).
- `rest-api/trace-context-header` (`rest-api/trace-context-header`, `HDR`) (REST) — `traceparent` W3C Trace Context.
...
- **R-OBS-LOG-X1.** **PII в логах.** Email, phone, ФИО, адрес, паспорт, токены, пароли — `observability/no-pii-in-logs` **критическое нарушение**. Маскировать (`***@***.com`) или вообще не логировать. См. `auth-patterns/no-pii-in-logs-and-events`.
...
- **R-OBS-TRC-2.** **`traceparent` propagation** (W3C Trace Context, см. `rest-api/trace-context-header`). Spring + OTel автоматически:
...
- **R-OBS-TRC-X4.** **Trace context разрывается на `@Async` без TaskDecorator**. См. `observability/context-propagated-to-async`.
...
- **R-OBS-HC-2.** **Custom HealthIndicator** для каждой критичной внешней системы (см. `resilience/cached-health-probe-per-system`):
  - DB через `DataSourceHealthContributor` — auto.
  - Redis через `RedisHealthIndicator` — auto.
  - External HTTP — custom HealthIndicator с TTL-кешем (см. `resilience/cached-health-probe-per-system`).
...
- **R-OBS-HC-X3.** **Health-probe делает business-операцию** (`registerTestOrder`). См. `resilience/cached-health-probe-per-system` — ddos самих себя через health-checks.
...
- **R-OBS-SLO-4.** **Alerts отдельные от SLO**:
  ...
  - Cache hit rate `< 70%` — `caching/cache-metrics-enabled`.
  - Kafka consumer lag — `kafka/consumer-lag-alerting`.

### 7.2 Запрещено

- **R-OBS-SLO-X1.** **Alert на каждый ERROR** в логах. Alert fatigue → команда игнорирует. Фильтруй: ERROR класса `ServiceUnavailableException` повторяющийся 100 раз в минуту = один алерт; единичные ошибки валидации входа и доменные отказы (`warn`) = не алерт.
...
|---|---|---|
| PII в логах (email, phone, токены) | `observability/no-pii-in-logs` (см. `auth-patterns/no-pii-in-logs-and-events`) | маскировать или не логировать |
| `System.out.println` / `e.printStackTrace()` | `observability/no-direct-stdout-logging` | `@Slf4j` log.error |
| String-concat в log args | `observability/parameterized-log-messages` | `{}` placeholders |
| `log.error("Failed: " + e.getMessage())` без stack | `observability/parameterized-log-messages` | `log.error("Failed: {}", ctx, e)` |
| Полный request body в логах | `observability/no-pii-in-logs` | только id |
| INFO на каждый HTTP-запрос | `observability/log-levels-and-noise` | access-log отдельно |
| `user_id` / `request_id` как metric tag | `observability/low-cardinality-labels` | бизнес-категории |
| Не-стандартизованные dimensions | `observability/standard-metric-dimensions` | `service`/`env`/`version` через `management.metrics.tags` |
| Micrometer без registry | `observability/metrics-are-exported` | Prometheus registry обязателен |
| `/actuator/prometheus` без auth публично | `observability/management-endpoints-restricted` | internal-only через network policy |
| Sampling 100% tracing в проде | `observability/sampling-strategy` | 1-10% + 100% errors |
| PII в span attributes | `observability/manual-spans-are-closed` | только internal IDs |
| Manual span без try-finally | `observability/manual-spans-are-closed` | try-with-resources / finally |
| Trace разрывается на @Async | `observability/context-propagated-to-async` (см. `observability/context-propagated-to-async`) | TaskDecorator |
| Business-state в health check | `observability/health-check-is-technical` | техническое состояние; бизнес → SLO |
| Liveness зависит от DB | `observability/liveness-and-readiness-split` | только readiness |
| Health-probe делает business-операцию | `observability/health-check-is-technical` | light probe |
| Exposing `/actuator/env` без auth | `observability/management-endpoints-restricted` | exposure: explicit list |
| Один port для business + actuator | `observability/separate-management-port` | management.server.port: 8081 |
| `exposure.include: '*'` в проде | `observability/management-endpoints-restricted` | explicit list |
| MDC без `MDC.clear()` в finally | `observability/context-set-at-edge-and-cleared` | try { ... } finally { MDC.clear(); } |
| `MDC.put` в произвольных местах | `observability/context-set-at-edge-and-cleared` | только в filter/interceptor |
| Alert на каждый ERROR | `observability/burn-rate-alerting` | по burn-rate |
| SLO без error budget | `observability/slo-with-error-budget` | 99.9% даёт 43min/month |
| Alerts без runbook | `observability/alerts-have-runbooks` | каждый alert = ссылка на runbook |
