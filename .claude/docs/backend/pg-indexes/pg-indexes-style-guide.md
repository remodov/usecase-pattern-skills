...
### `PG-I-011` — Порядок написания в `WHERE` не важен — оптимизатор сам переставляет

`WHERE c=3 AND b=2 AND a=1` эквивалентно `WHERE a=1 AND b=2 AND c=3`.

### `PG-I-012` — Первым в индексе — поле, по которому чаще всего идёт `=` в `WHERE`
...
### `PG-I-014` — `ORDER BY` — порядок и направление полей должны совпадать с индексом
...
### `PG-I-031` — Индекс на низкоселективную колонку (`is_deleted`, `status`, `country` в одной стране) бесполезен
...
### `PG-I-033` — На SSD `random_page_cost = 1.1`, не дефолтные 4.0
...
### `PG-I-035` — После массовой загрузки/UPDATE — `ANALYZE` руками
...
### `PG-I-023` — GiST — для range-типов, геометрии (PostGIS), kNN, EXCLUDE-constraint

Поддерживает `ORDER BY x <-> point` (kNN).
- GIN vs GiST для текста/массивов: GIN — быстрее на чтение, медленнее на запись; GiST наоборот.

### `PG-I-024` — BRIN — для огромных append-only таблиц с естественной упорядоченностью
...
### `PG-I-015` — Не создавай `(a)`, если уже есть `(a, b, c)` — последний покрывает запросы по `a`
...
### `PG-I-017` — PostgreSQL не строит индекс по FK автоматически. Если по FK джойнят / удаляют родителя — индекс нужен
...
### `PG-E-030` — Для диагностики — `EXPLAIN (ANALYZE, BUFFERS)`
...
### `PG-E-033` — Если важное условие в `Filter:`, а не в `Index Cond:` — индекс по этому полю не используется как ключ

Нужно либо новый индекс, либо изменить порядок полей в существующем.

### `PG-E-034` — `Heap Fetches > 0` на Index Only Scan → visibility map устарел

Помогает `VACUUM`.

### `PG-E-035` — `Recheck Cond:` показывает lossy bitmap
...
### `PG-E-037` — `Sort Method: external merge Disk:` → сортировка ушла на диск

Увеличь `work_mem` или добавь индекс с нужным порядком.

### `PG-E-038` — Параллельный план (`Gather`/`Workers Launched`) оправдан на больших таблицах
...
### `PG-E-010` — Запрос `SELECT count(1) FROM t WHERE c > X` при индексе `(a, b, c)` — типичный пример «индекс используется, но не как ключ»
...
### `PG-E-011` — Если на этом запросе нужна реальная скорость — нужен индекс по `c` (или `(c, ...)`)

Тогда план превратится в Index Scan с `Index Cond: (c > X)`.

### `PG-E-012` — Если `Heap Fetches > 0` — `VACUUM`
...
### `PG-E-040` — EXPLAIN — про план одного запроса в моменте
