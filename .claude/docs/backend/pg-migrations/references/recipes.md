# PostgreSQL Migrations — рецепты

Справочник к требованиям `../spec.md`: SQL-рецепты expand-contract, разбор ACCESS EXCLUSIVE-операций, `CONCURRENTLY`, `CHECK NOT VALID`. Требования адресуются `ID`, старый код `PG-M-NNN` оставлен в заголовках разделов, чтобы ссылки из архива ревью и suppression-комментариев продолжали находиться грепом.
