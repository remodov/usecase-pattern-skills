# Resilience — реализация

Свод правил защиты Java/Spring-сервисов команды UCP от отказов внешних систем: timeouts, circuit breaker, retry, bulkhead, fallback, health checks. Каждое правило идентифицируется кодом (`resilience/breaker-window-and-thresholds`, `resilience/breaker-on-adapter-method`) — скилл `ucp-resilience-review` цитирует эти коды в findings.
...
Гайд опирается на:
- `spring-bootstrap/single-persistence-mechanism`/`spring-bootstrap/prefer-generated-types` (`backend/java/spring-bootstrap/references/implementation.md`) — стек persistence (касается task-queue retry).
- `auth-patterns/money-commands-need-idempotency-key` (`auth-patterns/money-commands-need-idempotency-key`) (`backend/auth-patterns/references/java/implementation.md`) — `Idempotency-Key` для money-команд (определяет, можно ли retry).
- `R-OAS-*` (требования `rest-api/*`) — OpenAPI-first контракт (определяет связку с OpenAPI generator).
...
- **R-RES-WHERE-3.** Защита **schedulers и outbox-relay** делается через **task-queue retry** (DB-driven, см. `resilience/durable-retry-via-task-queue`), не через Resilience4j. Resilience4j покрывает in-memory транзиенты <5s; task-queue — durable retry для долгих отказов (>30s) и переживания рестарта сервиса.
...
- **R-RES-ISO-1.** На **каждую внешнюю систему** — отдельный `OkHttpClient` (или `RestClient` / `WebClient`) **с собственным**:
  ...
  - `Bulkhead` instance — изолирует concurrency (см. `resilience/bulkhead-is-semaphore-based`).
...
- **R-RES-TO-1.** Timeout hierarchy: `connectTimeout < readTimeout < callTimeout`.
  ...
  - `readTimeout` — между байтами ответа. Зависит от слоупока внешней системы. Типовые: `10s` (быстрые API), `30s` (тяжёлые), `60s` (отчёты/экспорт). Если `>60s` — задача для async-pattern (`resilience/durable-retry-via-task-queue`), не для sync-вызова.
...
- **R-RES-TO-2.** Timeouts конфигурируются per-system через `<system>ClientSettings` (`@ConfigurationProperties("client.<system>")`):
  ...
  client.odnakassa:
    ...
    max-concurrent: 30
  ```
  Расхождения от типовых (`resilience/timeout-hierarchy`) — комментарием в yml с обоснованием.

- **R-RES-TO-3.** Если на эндпоинте есть `traceparent` (см. `rest-api/trace-context-header` REST guide) и TimeBudget — адаптер уважает оставшееся время. При `remainingBudget < callTimeout` — `RequestsInterceptor` ставит client-side timeout = `min(callTimeout, remainingBudget - 100ms buffer)`.
...
- **R-RES-TO-X3.** `readTimeout > 60s` для синхронного вызова из HTTP-handler'а. Перевести в task-queue (`resilience/durable-retry-via-task-queue`) или async-pattern (`rest-api/long-running-via-polling` REST guide).
...
- **R-RES-CB-1.** `@CircuitBreaker(name = "<system>")` — на **public-методе out-adapter**, который вызывает внешнюю систему. Не на generated client (см. `resilience/client-generated-from-contract`), не на handler'е, не на репозитории.
  ```java
  @Component
...
- **R-RES-RE-1.** `@Retry(name = "<system>")` допустим **только** при одном из условий:
  ...
  2. Команда выполняется с `Idempotency-Key` (см. `auth-patterns/money-commands-need-idempotency-key`). Внешняя система обязана сама дедуплицировать.
  ...
  resilience4j.circuitbreaker:
    ...
    instances:
      ...
      odnakassa:
        base-config: default
  ```
- **R-RES-CFG-3.** Имена instances — same as system: `sber`, `odnakassa`, `insurance`, `receipt`. Совпадают с именами beans (`resilience/instance-names-match-system`).
...
- **R-RES-ASYNC-1.** Если внешняя система требует polling (как страхование из bus-tickets — «отправили запрос → ждём результат»), polling реализуется через **task-queue**, не через `Thread.sleep` в синхронном handler'е:
  ...
  - Возвращает клиенту `202 Accepted` (см. `rest-api/long-running-via-polling` REST guide).
...
- **R-RES-OBS-3.** Логирование — структурированное (см. требования `observability/*`, в планах). При каждом state-transition CB — лог уровня WARN с system, prev_state, new_state, failure_rate. Не на каждый успешный call.
...
|---|---|---|
| Shared OkHttpClient/pool на разные системы | `resilience/client-per-external-system` | per-system bean + pool |
| `OkHttpClient` без timeouts | `resilience/timeout-hierarchy` | `connect/read/call` явно |
| `callTimeout < readTimeout` | `resilience/timeout-hierarchy` | `callTimeout ≥ connect+read+buffer` |
| Sync `readTimeout > 60s` | `resilience/no-long-synchronous-waits` | task-queue |
| `@CircuitBreaker` на репозитории / JOOQ-вызове | `resilience/no-protection-around-local-operations`, `resilience/no-protection-around-local-operations` | только на adapter-методе с outbound HTTP |
| Custom CB на try/catch + counter | `resilience/no-custom-breaker` | Resilience4j |
| `@CircuitBreaker(name = "default")` для всех систем | `resilience/instance-names-match-system` | per-system name |
| `@Retry` на write без Idempotency-Key | `resilience/retry-only-when-safe` | только GET или с Idempotency-Key |
| `@Retry` на 4xx | `resilience/retry-only-when-safe` | только на 5xx + IOException |
| `@Retry` без exp backoff | `resilience/retry-with-backoff-and-limit` | `enable-exponential-backoff: true` |
| `@Retryable` Spring-Retry для outbound | `resilience/retry-with-backoff-and-limit` | Resilience4j Retry |
| Thread-pool bulkhead для outbound | `resilience/bulkhead-is-semaphore-based` | semaphore-based |
| Fallback с `null` / `Money.ZERO` для money | `resilience/fallback-does-not-fake-success` | task-queue + 202 Accepted |
| Тихий fallback с success | `resilience/fallback-does-not-fake-success` | явный fallback с queueing |
| Каскадный fallback в другой провайдер без CB | `resilience/fallback-does-not-fake-success` | каждый провайдер — свой CB |
| Программный `CircuitBreakerConfig.custom()` | `resilience/declarative-configuration` | `application.yml` declarative |
| Аннотации на generated `<X>Api` interface | `resilience/breaker-on-adapter-method` | на adapter-методе |
| CB в `executeCall<T>` helper с backendName строкой | `resilience/breaker-on-adapter-method` | аннотации на adapter-методе |
| Возврат generated DTO из port-метода | `R-RES-OAS-X3` | mapper → domain |
| Sync health-probe без кеша | `resilience/cached-health-probe-per-system` | TTL 30s |
| Health-probe с business-операцией | `resilience/cached-health-probe-per-system` | light GET/OPTIONS |
| `Thread.sleep` цикл в sync-handler'e | `resilience/no-long-synchronous-waits` | task-queue |
| `Thread.sleep > 5s` в коде | `resilience/no-long-synchronous-waits` | task-queue |
| Отключение R4J metrics | `resilience/resilience-state-is-observable` | оставить включённым |
...
| Шаг | Что | Эффект |
|---|---|---|
| 1 | **Per-system OkHttpClient** (`resilience/client-per-external-system`) — разделить shared dispatcher/pool по системам | Изоляция: зависание Sber не блокирует OdnaKassa |
| 2 | **`@CircuitBreaker` на out-adapter методах** (`resilience/breaker-on-adapter-method`) с `application.yml` конфигом | Fast-fail при отказе системы вместо timeout-чейна |
| 3 | **`@Bulkhead` per-system** (`resilience/bulkhead-is-semaphore-based`) с semaphore | Раннее отвержение новых call'ов до исчерпания pool |
| 4 | **HealthIndicator per-system** (`resilience/cached-health-probe-per-system`) с TTL-кешем | K8s знает «у нас Sber лёг» через 30s, не через 5min логов |
| 5 | **Sleep-loop polling → task-queue** (`resilience/async-calls-need-time-limit`) — переписать `InsuranceClientAdapter` polling на DB-driven | Освобождение worker-threads под нагрузкой |
| 6 | **Новые out-adapter — `spring-restclient`** (`resilience/client-generated-from-contract`), Retrofit2 — только в legacy-сервисах | Native Spring backend/observability/tracing без обвязки |
