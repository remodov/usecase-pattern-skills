...
Каждое правило идентифицируется кодом (`rest-api/path-lowercase-kebab-case`, `rest-api/error-status-codes-limited` и т. п.). Скиллы `ucp-api-design` и `ucp-api-review` цитируют эти коды в findings.
...
- **R-MTH-6.** Коды успешных ответов соответствуют семантике метода:
  ...
  Полный перечень допустимых кодов ошибок — в правиле `rest-api/error-status-codes-limited`.
...
- **R-NEST-4.** Path-переменная для идентификатора в дизайне URL всегда называется `{id}`. Контекст (имя ресурса в предыдущем сегменте) устраняет неоднозначность.
  ...
  > **Двойной стандарт с OpenAPI:** в спецификации OpenAPI параметры пути обязаны быть уникально именованы (`orderId`, `itemId`) — это требование инструмента (Swagger/Redoc не работают с одинаковыми именами). См. правило `rest-api/unique-path-parameter-names`.
...
- **R-VER-5.** Клиент API ОБЯЗАН игнорировать (или явно обрабатывать как unknown) неизвестные значения enum и неизвестные поля в ответе. На этом обязательстве основано правило `rest-api/new-version-only-for-breaking-change`: добавление нового enum-значения или нового поля — non-breaking. Сервер развивает API без новой версии при условии, что клиенты следуют этому правилу.
- **R-VER-6.** Перечень изменений по типам.
  ...
  **Не ломающие (допустимы в текущей версии, опираются на `rest-api/client-tolerates-unknown-values`):**
...
- **R-QRY-4.** Offset-based пагинация: параметры `page` (1-based) и `size`.
  ...
  **Реализация:** контракт API — 1-based; в Spring Data конфигурируется `spring.data.web.pageable.one-indexed-parameters=true`. Ручная конвертация `page-1` в коде запрещена — теряется единая точка истины (детали — в требованиях `spring-bootstrap/*`).
...
- **R-QRY-9.** Сложные поисковые запросы — `POST /resources/search` с телом JSON.
  ...
  **Правила POST-поиска:**
  ...
  5. Идемпотентность: POST-поиск идемпотентен по факту, но HTTP этого не гарантирует — при необходимости кеширования добавь заголовок `Idempotency-Key` (см. `rest-api/idempotency-key-header`).
...
- **R-FLD-5.** Идентификаторы — суффикс `Id`.
  ...
  ```
- **R-FLD-6.** В теле PATCH-запроса (JSON Merge Patch, [RFC 7396](https://www.rfc-editor.org/rfc/rfc7396.html)) явный `null` — команда удалить поле. Это семантика **запроса**, не нарушение `rest-api/no-nulls-in-successful-response` (правило про `null` в ответе).
  ```http
...
- **R-RSP-2.** Коллекция — `{ "items": [...] }` плюс метаданные пагинации на том же уровне (см. `rest-api/pagination-forms`/`rest-api/pagination-forms`). Поле с данными всегда называется `content` — это не envelope, а структура пагинированного ответа.
...
- **R-RSP-X1.** Поля со значением `null` в успешном ответе (2xx). Если поле не заполнено — оно отсутствует в JSON. Это уменьшает размер ответа и упрощает контракт.
  ...
  > Семантика `null` в **запросах** PATCH (JSON Merge Patch) — это команда удалить поле. См. `rest-api/explicit-null-in-patch-means-delete`.
...
- **R-RSP-X4.** Envelope-обёртка для единичных ресурсов.
  ...
  ```
  Причины: HTTP-статус уже сообщает об успехе/ошибке; ошибки возвращаются в формате RFC 9457 (раздел 13); коллекции оборачиваются в `{ "items": [...] }` — это не envelope, а структура пагинации.
...
- **R-HDR-2.** Кастомные заголовки используют доменный префикс, единый для всех сервисов проекта/компании. Префикс выбирается один раз и фиксируется в стандартах команды (в примере — `Shop-`):
  ...
  ```
  - `Shop-Request-Id` — идентификатор конкретного запроса от клиента (для дедупликации и логирования). Не путать с `traceparent` (см. `rest-api/trace-context-header`).
...
- **R-HDR-4.** Для распределённой трассировки используется стандарт [W3C Trace Context](https://www.w3.org/TR/trace-context/). Заголовок `traceparent` передаётся клиентом или генерируется на входе и прокидывается через все сервисы в цепочке.
  ...
  - `trace-id` из `traceparent` используется как `traceId` в теле ошибки RFC 9457 (см. `error-handling/exceptions-are-part-of-contract`).
...
- **R-ERR-1.** Тело ошибки соответствует структуре RFC 9457:
  ...
  Атрибуты:
  - **`type`** — стабильный URI/URN, идентифицирующий **категорию** ошибки. Подробнее в `error-handling/domain-and-validation-mapping`.
  - **`status`** — HTTP-статус ответа.
  - **`title`** — короткое описание (обычно совпадает с названием статуса).
  - **`instance`** — уникальный идентификатор инцидента (URI/URN).
  - **`traceId`** — ID трассировки (из заголовка `traceparent`, см. `rest-api/trace-context-header`).
...
- **rest-api/error-body-follows-standard.** Content-Type ответа с ошибкой — `application/problem+json`.

- **rest-api/error-codes-enumerated.** `code` формируется как UPPER_SNAKE_CASE. Все возможные коды перечислены в OpenAPI как enum.
  ```
...
- **rest-api/validation-errors-list-violations.** Ошибки валидации полей возвращаются с HTTP-кодом `400 Bad Request`, `code = VALIDATION_ERROR` и расширением `violations` — массивом ошибок по конкретным полям:
  ```json
  {
    "type": "urn:problem:order-service:validation-error",
    "status": 400,
...
- **rest-api/validation-errors-list-violations.** Правила формирования `violations`:
...
- **rest-api/error-codes-enumerated.** OpenAPI-схема `ProblemDetails` и `Violation`. ENUM-список `ErrorCode` формируется один на контракт.
  ```yaml
  ErrorCode:
    type: string
...
- **rest-api/error-examples-in-operations.** Все доступные ошибки указаны как `examples` в объектах ответа каждого метода. Полные перечни ниже.

- **rest-api/error-status-codes-limited.** Используемые HTTP-коды ошибок:
  - `400` и `500` — всегда
...
- **rest-api/error-body-follows-standard.** Content-Type `application/json` для тела ошибки. Используй `application/problem+json`.

- **rest-api/error-type-is-stable-category.** `type: "about:blank"`. Теряется машиночитаемая категория ошибки. Если портала документации нет — используй URN формы `urn:problem:<service>:<code>` (см. `error-handling/catch-in-three-places-only`).

- **rest-api/error-status-codes-limited.** HTTP-коды ошибок вне списка `rest-api/error-status-codes-limited` (например, `418`, `422`, `451`). Если возникает потребность — обсуждай в архитектурном комитете.

- **rest-api/no-internals-in-error-body.** Stack traces, SQL-запросы, внутренние пути в теле ошибки `500`.
...
- **R-OAS-3.** Параметры пути в OpenAPI именуются уникально по контексту: `{orderId}`, `{itemId}`. В дизайн-документации (см. `rest-api/path-parameter-naming`) используется `{id}` — контекст устраняет неоднозначность; в OpenAPI это требование инструмента (Swagger/Redoc не работают с одинаковыми именами параметров).
  ```yaml
...
| CamelCase в пути | `rest-api/path-lowercase-kebab-case` | `/order-items` |
| snake_case в пути | `rest-api/path-lowercase-kebab-case` | `/order-items` |
| Завершающий слеш | `rest-api/path-lowercase-kebab-case` | `/api/v1/orders` |
| Расширение файла в пути | `rest-api/path-lowercase-kebab-case` | `/api/v1/orders` |
| ID в теле вместо пути | `rest-api/nesting-max-two-levels` | `PUT /orders/{id}` |
| Глубокая вложенность | `rest-api/nesting-max-two-levels` | `/comments?itemId={id}` |
| Множественное/единственное число вперемешку | `rest-api/collections-plural-singletons-singular` | `/orders/{id}/items` |
| GET с побочным эффектом | `rest-api/methods-match-semantics` | `POST /orders/{id}/cancel` |
| Версия в query | `rest-api/version-in-path` | `/api/v1/orders` |
| Минорная версия в пути | `rest-api/version-in-path` | `/api/v1/...` |
| Бизнес-логика в query | `rest-api/filters-ranges-and-search` | `POST /orders/{id}/cancel` |
| Comma-separated массивы в query | `rest-api/arrays-as-repeated-parameters` | повтор параметра |
| `page=0` в публичном контракте | `rest-api/pagination-forms` | `page=1` |
| Префикс `X-` в заголовках | `rest-api/headers-standard-and-prefixed` | доменный префикс (`Shop-`) |
| Envelope-обёртка | `rest-api/single-resource-is-flat` | плоский ресурс |
| `null` в успешном ответе | `rest-api/no-nulls-in-successful-response` | отсутствие поля |
| `nullable: true` в OpenAPI | `rest-api/no-nulls-in-successful-response` | `required` или отсутствие |
| `application/json` для ошибок | `rest-api/error-body-follows-standard` | `application/problem+json` |
| `type: "about:blank"` в ошибках | `rest-api/error-type-is-stable-category` | URN `urn:problem:<service>:<code>` |
| Stack traces в теле `500` | `rest-api/no-internals-in-error-body` | `code` + общий `detail` |
| Rate limiting без заголовков | `rest-api/rate-limit-headers` | `Retry-After` + `RateLimit-*` |
| Deprecation без `Sunset` | `rest-api/deprecation-with-sunset` | заголовок `Sunset` с датой |
| `me` для собственных ресурсов | `rest-api/me-alias-only-when-ambiguous` | контекст из токена |
| HATEOAS-ссылки в теле | `rest-api/no-hateoas-links-in-body` | OpenAPI-описание навигации |
| Любой метод кроме POST для action | `rest-api/action-endpoints-shape` | `POST /orders/{id}/confirm` |
