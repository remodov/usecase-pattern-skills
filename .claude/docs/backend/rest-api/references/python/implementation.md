# REST API — реализация на Python (FastAPI, contract-first codegen)

Реализация язык-нейтрального контракта `../spec.md` на FastAPI. Коды общие с Java. **Бо́льшая часть
...
**не пишутся руками** — правило «не править сгенерированное» действует (`codegen/generated-files-not-edited`), правки только в `ApiBaseModel`
поверх генерации (`codegen/shared-base-model`). Сгенерированные схемы дают `snake_case` поля + `camelCase` alias, `populate_by_name`,
...
- `R-URL-*` — путь в декораторе (`@router.get("/orders/{order_id}/items")`), kebab-case, без trailing-slash
  (`redirect_slashes=False`, иначе FastAPI редиректит). `rest-api/version-in-path` — `APIRouter(prefix="/api/v1")`.
- `R-MTH-*` — метод = декоратор (`@router.post`); статусы — `status_code=201`. `rest-api/action-endpoints-shape` — action всегда `POST`
  (`POST /orders/{id}/confirm`).
- `rest-api/path-parameter-naming`/`rest-api/unique-path-parameter-names` — path-параметр в FastAPI именуется уникально (`{order_id}`), это требование инструмента;
  в дизайне — `{id}`.
...
true`), **не** comma-separated (`rest-api/arrays-as-repeated-parameters`); сложный поиск — `POST /resources/search` (`rest-api/filters-ranges-and-search`).
...
> `?status=CREATED,CONFIRMED` (CSV) — `rest-api/arrays-as-repeated-parameters`. FastAPI с `list[...]` = `?status=CREATED&status=CONFIRMED`.
...
`rest-api/json-field-naming` — camelCase через Pydantic `model_config = ConfigDict(alias_generator=to_camel, populate_by_name=True)`.
`rest-api/json-field-naming` — даты ISO 8601 (Pydantic сериализует `datetime` так по умолчанию). `rest-api/json-field-naming` — enum-значения
UPPER_SNAKE (`class OrderStatus(StrEnum)`). `rest-api/no-nulls-in-successful-response` — **нет `null` в 2xx**: `response_model_exclude_none=True`
на маршруте / `model_config exclude_none`. `rest-api/collection-response-shape` — коллекция `{ "items": [...] }` + метаданные пагинации.
`rest-api/status-codes-and-bodies` — `201` + `Location`-заголовок + тело-ресурс. `rest-api/no-nulls-in-successful-response` — пустая коллекция `[]`, не `null`. `rest-api/no-nulls-in-successful-response` —
...
> `discount=None`/`comment=None` **не** попадут в 2xx-ответ благодаря `exclude_none` (`rest-api/no-nulls-in-successful-response`); поле либо есть,
...
422 FastAPI — переопредели handler); не светить stack/SQL в `500` (`rest-api/no-internals-in-error-body`). `R-RATE-*` — `429` + `Retry-After`
...
> на `400` + problem+json (`rest-api/validation-errors-list-violations`, `rest-api/error-status-codes-limited` запрещает `422`).

```python
# Маппинг дефолтного FastAPI 422 → 400 VALIDATION_ERROR + problem+json (rest-api/validation-errors-list-violations; полный handler — в backend/validation/python)
@app.exception_handler(RequestValidationError)
async def on_validation_error(request: Request, exc: RequestValidationError) -> Response:
    violations = [                                            # rest-api/validation-errors-list-violations: все ошибки, dot-path вложенных полей
        {"field": ".".join(str(p) for p in e["loc"] if p != "body"), "message": e["msg"]}
        for e in exc.errors()
    ]
    body = {
        "type": "urn:problem:order-service:validation-error",
        "status": 400, "title": "Bad Request",               # rest-api/error-status-codes-limited: НЕ 422
        "detail": "Ошибка валидации входных данных",
        "traceId": get_trace_id(),
        "code": "VALIDATION_ERROR",                           # rest-api/error-codes-enumerated: UPPER_SNAKE
        "violations": violations,
    }
    return JSONResponse(status_code=400, content=body,
                        media_type="application/problem+json")  # rest-api/error-body-follows-standard
```

Тело 2xx/4xx — пример `400` problem+json, который вернёт handler выше (`rest-api/validation-errors-list-violations`/`rest-api/validation-errors-list-violations`):
...
`GET /api/v1/tasks/{id}`. `R-LOC-*` — `Accept-Language`, дефолт `ru`, не локализовать enum/URI (`rest-api/localization-via-accept-language`).

## OpenAPI-метаданные (`R-OAS-*`)

`rest-api/operation-id-and-tags` — `operation_id` в `camelCase` на каждом маршруте (`@router.post(..., operation_id="createOrder")`) —
FastAPI иначе генерирует длинные авто-id. `rest-api/operation-id-and-tags` — `tags=["Orders"]`, action к тегу родителя. `rest-api/operation-has-summary` —
`summary`/`description` в декораторе. Контракт = Pydantic-модели (не голый `dict`, cross-ref `validation/controller-implements-generated-contract`).
...
Из него генерируются Pydantic v2-схемы (не пишутся руками, `codegen/schemas-generated-from-contract`):
...
Сгенерированный артефакт (соответствует разделу JSON выше; ручная база `ApiBaseModel` — `codegen/shared-base-model`):
