# REST API — индекс правил (язык-нейтральный)

> **Что это.** Сжатый индекс правил: код + интент, по разделам — **общий контракт для всех языков**.
> Рабочий вход скиллов: review цитирует код, design сверяется по чек-листу. Бо́льшая часть правил —
> протокольного уровня (URL/методы/статусы/ошибки/пагинация) и идентична на всех языках.
> **Реализация по языкам** — `java/rest-api-style-guide.md` (Spring + OpenAPI contract-first) и
> `python/rest-api-style-guide.md` (FastAPI code-first: Pydantic+декораторы → OpenAPI); открывай нужный точечно.
> Коды: `<PREFIX>-<N>` — обязательно, `<PREFIX>-X<N>` — запрещено. **Коды общие для всех языков** — ключевое
> отличие: contract-first (генерация сервера из OpenAPI) ↔ code-first (OpenAPI генерируется из кода).
