# Background Jobs & Scheduling — реализация на Python (FastAPI / SQLAlchemy 2.0 async)

Реализация контракта `../spec.md` (`R-JOB-*`). Те же коды и разделы — здесь идиома на async-стеке.
...
- **AVOID** `BackgroundTasks` для денег/периодики/гарантий (`scheduler/mechanism-matches-work`).
...
- **AVOID** опоры на «очередь доставит ровно один раз» (`scheduler/job-is-idempotent`).
...
Для beat-singleton: один экземпляр beat (replicas=1) или single-beat lock — иначе тик задвоится (`scheduler/single-source-of-tick`).
Альтернатива без Celery — k8s `CronJob` с `concurrencyPolicy: Forbid`, args `curl -XPOST .../internal/jobs/process`.

- **AVOID** in-process `APScheduler`/`while`-loop в каждой реплике без координации (`scheduler/single-source-of-tick`). APScheduler допустим **только** для single-replica/dev; в multi-replica обязателен k8s `Lease`/leader-election или PG advisory-lock.
- **AVOID** расчёта «воркер один» вместо явного claim (`scheduler/work-unit-claimed-atomically`); нескольких beat-экземпляров без single-lock.

## 4. Надёжность (`R-JOB-REL-*`)

- Retry с экспоненциальным backoff, лимит попыток, затем **DLQ** (см. `resilience`/`R-RES-RETRY-*`).
- Транспортная ошибка внешней системы → единица остаётся `PENDING`/`IN_PROGRESS` (заберёт следующий тик); бизнес-ошибка → терминальный `FAILED` (`scheduler/transient-versus-business-errors`).
- Зависшие claim'ы восстанавливать по visibility-timeout (`run_after`/`locked_until` в прошлом снова видны).
- **Redis reliable-queue:** processing-list/consumer-group **per-replica**, иначе `recover()` вернёт в общую очередь то, что обрабатывает другая реплика (`scheduler/stuck-claims-are-recovered`).
- **AVOID** blocking `asyncio.sleep`-retry внутри одного прохода, удерживающего claim (`scheduler/bounded-retries-then-parking`).
...
Колонки времени — `DateTime(timezone=True)` (cross-ref `sqlalchemy/precise-column-types`). Конфиг — `pydantic-settings`:
...
- **AVOID** naive `datetime.now()` для cutoff (`scheduler/time-is-timezone-aware`); сырые `os.environ[...]` россыпью.
