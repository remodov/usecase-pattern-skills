# Streaming & Real-time — реализация на Python (FastAPI / Starlette)

Реализация контракта `../spec.md` (`R-STREAM-*`). Те же коды и разделы — здесь идиома на FastAPI/Starlette.
Стек: FastAPI `WebSocket`, `sse-starlette` (`EventSourceResponse`), `StreamingResponse`, `redis.asyncio` pub/sub для
fan-out. Async-дисциплина — по `python/async/spec.md` (`PYASYNC-*`).
...
- **AVOID** WebSocket, где хватает SSE (`streaming/mechanism-matches-exchange`); формирование всего ответа в память (`streaming/large-response-is-streamed`).
...
- **AVOID** приём соединения без проверки токена (`streaming/authenticate-on-handshake`); бессрочное соединение без переучёта `exp` (`streaming/long-lived-connections-recheck-rights`).
...
- **AVOID** неограниченной очереди исходящих (`streaming/backpressure-is-bounded`).

## 4. Heartbeat / idle-timeout (`streaming/heartbeat-and-idle-timeout`)

- SSE — `EventSourceResponse(gen(), ping=15)` (ping-комментарий каждые 15 c).
- WebSocket — периодический `await ws.send_json({"type": "ping"})` + закрытие при отсутствии pong/активности по таймауту (`asyncio.timeout`, cross-ref `async/external-waits-have-timeouts`).
- **AVOID** соединения без heartbeat/idle-timeout (`streaming/heartbeat-and-idle-timeout`).
...
- БД-доступ — короткими сессиями per-message/per-chunk через репозиторий, **не** одна транзакция на всё соединение (`streaming/no-resources-held-for-connection-lifetime`, cross-ref `sqlalchemy/transaction-boundary-on-handler`).
- **AVOID** записи в закрытый канал (`streaming/disconnect-cancels-work`); открытой транзакции на всю длину соединения (`streaming/no-resources-held-for-connection-lifetime`).

## 6. Масштабирование (`streaming/fanout-works-across-instances`)
...
- **AVOID** broadcast из локального in-memory реестра соединений (`streaming/fanout-works-across-instances`).
...
- Лог — событие соединения + идентификаторы (`order_id`, `principal.sub`), **без** payload/PII (`streaming/connections-observable-without-payloads`, cross-ref `observability/no-pii-in-logs`).
