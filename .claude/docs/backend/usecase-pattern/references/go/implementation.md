# usecase-pattern — реализация на Go (net/http + chi)

Реализация контракта `../spec.md` (`R-UC-*`/`R-HND-*`/`R-DSP-*`/`R-CQRS-*`/`R-LAY-*`/`R-HEX-*`/`R-STEP-*`/`R-TX-*`) на Go-стеке (stdlib `net/http` + chi).
...
`usecase-pattern/usecase-implements-marker` / `usecase-pattern/usecase-is-immutable-carrier` — UseCase = immutable struct без методов-логики. Маркеры команды/запроса — интерфейсы-маркеры с приватным методом, параметризованные через дженерик-обёртку:
...
`usecase-pattern/explicit-result-type` — результат явно типизирован через тип-параметр маркера; для «пустого» ответа — отдельный тип:
...
`usecase-pattern/usecase-is-immutable-carrier` ❌ логика в UseCase-структуре (методы с обращением к БД, вычисления). `usecase-pattern/one-usecase-one-operation` ❌ один struct на create+update — разнести. `usecase-pattern/usecase-is-immutable-carrier` ❌ изменяемые поля (slice-pointer без copy) — передавать через конструктор или явные поля; намеренная иммутабельность в Go — без сеттеров и без экспорта полей-указателей. `usecase-pattern/explicit-result-type` ❌ возвращать `error` как единственный результат без явного `VoidResult` там, где контроллер ожидает типизированный `R`.

---

## 2. Handler — `R-HND-*`

`usecase-pattern/one-handler-one-usecase` — Handler реализует типизированный интерфейс `Handler[UC, R]`:
...
`usecase-pattern/handler-registered-in-container` — Handler регистрируется в dispatcher'е при конструкторной сборке (`app/wire.go` / `app/di.go`); dispatcher находит его по типу UseCase.

`usecase-pattern/transaction-boundary-on-handler` — граница транзакции на Handler: команда открывает транзакцию через `UnitOfWork`, запрос — read-only (см. §8).

`usecase-pattern/one-handler-one-usecase` — один Handler — один UseCase (`CreateOrderHandler` только для `CreateOrder`).

`usecase-pattern/handler-is-stateless` — все зависимости через конструктор; поля — приватные, инициализируются один раз:
...
`usecase-pattern/handlers-do-not-call-handlers` ❌ Handler зовёт другой Handler напрямую — только через dispatcher / Step. `usecase-pattern/infrastructure-errors-become-domain` ❌ инфраструктурные ошибки (`pgconn.PgError`, `net.Error`) вылетают из Handler — маппить в доменные в адаптере (cross-ref `R-ERR-WHERE-2b`, `error-handling/go`). `usecase-pattern/handler-is-stateless` ❌ изменяемые поля в Handler (кэш запросов, счётчик) — Handler stateless.

---

## 3. Dispatcher и контроллер — `R-DSP-*`

`usecase-pattern/entry-calls-dispatcher` / `usecase-pattern/single-dispatcher` — контроллер не вызывает Handler напрямую, только через `Dispatcher`. В Go — реестр `reflect.Type → handlerFunc` собирается при старте:
...
`usecase-pattern/controller-maps-and-dispatches` ❌ бизнес-логика в контроллере (`if product.Stock == 0 { ... }`), обращения к репозиторию. `usecase-pattern/no-transport-objects-in-usecase` ❌ передача `*http.Request` / `auth.Principal`-объекта в UseCase — извлекать `UserID`/`TenantID` в контроллере.

---

## 4. CQRS — `R-CQRS-*`

`usecase-pattern/command-versus-query`/`-3` — команды реализуют `Command[R]` (имя-глагол: `CreateOrder`, `CancelOrder`, `ConfirmPayment`), запросы — `Query[R]` (`FindOrderByID`, `SearchOrders`, `GetCustomerBalance`).

`usecase-pattern/transaction-boundary-on-handler` — команда открывает read-write транзакцию через `UnitOfWork`; запрос выполняется в read-only соединении или read-only транзакции (см. §8):
...
`usecase-pattern/reads-via-read-model` — чтения возвращают read-модель (`OrderView`, `OrderPage`, `CustomerSummary`) через `ViewRepository`; запись — через `OrderRepository` с агрегатом.

`usecase-pattern/command-returns-minimum` ❌ команда возвращает `OrderView` со всеми связями — только `OrderID` / минимальный summary. `usecase-pattern/query-does-not-mutate` ❌ запрос пишет (обновляет `last_viewed_at`, счётчик просмотров) — это команда.

---

## 5. Слои моделей — `R-LAY-*`

`usecase-pattern/layer-models-do-not-leak` — на входе UseCase — поля из API-DTO или явные VO, не sqlc-struct из БД:
...
`usecase-pattern/layer-models-do-not-leak` — на выходе UseCase — read-DTO/VO, не sqlc-struct:
...
`usecase-pattern/explicit-mapper-between-layers` — маппинг — явными функциями в том слое, которому принадлежит преобразование; не один тип на все слои:
...
`usecase-pattern/layer-models-do-not-leak` ❌ sqlc-struct (`db.Order`) уходит напрямую в JSON-ответ через контроллер. `usecase-pattern/explicit-mapper-between-layers` ❌ «универсальный» маппинг через `json.Marshal` → `json.Unmarshal` или `reflect`-копирование. `usecase-pattern/layer-models-do-not-leak` — доменные агрегаты (`core/order/Order`) не утекают в HTTP-ответы (cross-ref `ddd-tactical/go`).

---

## 6. Hexagonal (Уровень 3) — `R-HEX-*`

`hexagonal/module-per-part` — раскладка пакетов:
...
`hexagonal/core-free-of-framework` — `core/` импортирует только stdlib и свои пакеты; **не** `net/http`, `pgx`, `chi`, `kafka-go`. Проверяется тестом на импорты или `go-arch-lint`:
...
`hexagonal/outbound-port-interface-in-core` — все внешние взаимодействия за интерфейсами-портами в `core/<bc>/port/`; реализация в `adapters/out/`:
...
`usecase-pattern/one-usecase-many-inbound-adapters` — Handler вызывается из HTTP-адаптера, Kafka-consumer-а и scheduler-а; Handler не дублируется:
...
`hexagonal/outbound-port-interface-in-core` ❌ прямой `pgxpool.Pool` в `core/`. `hexagonal/core-free-of-framework` ❌ `import "github.com/go-chi/chi/v5"` или `import "github.com/jackc/pgx/v5"` в `core/`. Enforce — тест на импорты или `go vet -mod=mod`.

---

## 7. Step — `R-STEP-*`

`usecase-pattern/step-for-reuse-only` — Step = интерфейс с одним методом `Execute`; stateless:
...
`usecase-pattern/step-for-reuse-only` — Step вводится, когда одна логика нужна ≥ 2 Handler-ам (например, `ValidateInventoryStep` используется в `CreateOrderHandler` и `ReserveStockHandler`).

`usecase-pattern/steps-flat-and-stateless` ❌ Step внутри Step — это уже логика Handler-а. `usecase-pattern/steps-flat-and-stateless` ❌ Step с полями, меняющимися между вызовами.

---

## 8. Транзакции и события — `R-TX-*`

`usecase-pattern/transaction-boundary-on-handler` — граница транзакции на Handler через `UnitOfWork`-порт; репозиторий не знает о транзакции снаружи него:
...
`usecase-pattern/one-usecase-one-transaction` — один UseCase = одна транзакция; Saga — оркестратор в Handler, каждый шаг — отдельный UseCase / внешний вызов с Outbox (cross-ref `distributed/go`):
...
`usecase-pattern/publish-events-after-save` — публикация доменных событий (Уровень 3) — после `repository.Add/Save`, затем `aggregate.ClearEvents()` (cross-ref `ddd-tactical/go`):
