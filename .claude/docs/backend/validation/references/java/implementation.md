...
Связанные стандарты:
- `rest-api/validation-errors-list-violations`/`rest-api/validation-errors-list-violations` (требования `rest-api/*`) — формат `violations` в ProblemDetails при `code=VALIDATION_ERROR`.
- `rest-api/operation-id-and-tags` (`rest-api/operation-id-and-tags`, `OAS`) (требования `rest-api/*`) — OpenAPI-first контракт.
- `resilience/client-generated-from-contract` (`resilience/client-generated-from-contract`) (требования `resilience/*`) — `useBeanValidation=true` опция openapi-generator для автоматической генерации constraints.
- `rest-api/localization-via-accept-language` (`rest-api/localization-via-accept-language`, `LOC`) (требования `rest-api/*`) — `message` в violations локализуется.
...
- **R-VLD-WHERE-1.** **Входной HTTP DTO** на контроллере → Jakarta Validation через `@Valid` на параметре. Это первая линия защиты, до того как невалидные данные дойдут до handler-а.
  ...
  ```
  Spring сам бросит `MethodArgumentNotValidException` → `@RestControllerAdvice` маппит в `400 Bad Request` с `code=VALIDATION_ERROR` и `violations` (см. `rest-api/validation-errors-list-violations`).
...
- **R-VLD-WHERE-3.** **Доменные инварианты** Aggregate — НЕ через Jakarta. Aggregate сам гарантирует целостность через методы:
  ...
  public void confirm() {
      if (status != OrderStatus.CREATED) {
          throw new OrderException.InvalidStateTransition(status, "confirm");
      }
      if (items.isEmpty()) {
          throw new OrderException.EmptyOrder(id);
      }
      this.status = OrderStatus.CONFIRMED;
      this.confirmedAt = DateTimeUtil.currentDateTime();
  }
  ```
  Бросает вариант доменного `sealed`-семейства (`OrderException`), который `@RestControllerAdvice` входного адаптера через исчерпывающий `switch` мапит в `409 Conflict` / `422` с `ErrorCode` контракта (`INVALID_STATE_TRANSITION`, `ORDER_EMPTY`). См. также `R-ENT-*`/`R-AGG-*` в требованиях `ddd-tactical/*`.

- **R-VLD-WHERE-5.** **Копия чужих данных** — справочник, который сервис реплицирует из системы-владельца (менеджеры, операторы, тарифы из CSMS), — сохраняется как пришла. Проверяются только идентификатор и метка версии: без них запись некуда положить и не с чем сравнить.

  ```java
  // нет: правило владельца перепроверяется на приёме, запись уходит в разбор
  public ManagerSnapshot {
      if (username == null || username.isBlank()) {
          throw new ManagerException.AttributeRequired("username");
      }
  }

  // да: проверяем только то, без чего запись не положить и не упорядочить
  public ManagerSnapshot {
  ...
  Пустое имя у менеджера — забота CSMS. Отбраковка на приёме означает, что менеджера в хабе нет вовсе: его токен получит отказ, а причину будут искать в правах, а не в очереди разбора. `@Valid` и Jakarta-аннотации применяются к тому, что сервис принимает **от клиента**, а не к тому, что он повторяет за источником (`validation/replica-is-stored-as-received`).
...
- **R-VLD-CC-2.** Расположение кастомных constraints:
  - **Доменно-специфичный** (`@VatNumber`, `@Iso8601Duration`) → `core/validation/` (часть domain-vocabulary).
...
Связка с требования `rest-api/*`: контроллеры implements generated `<Tag>Api`, DTO генерируются `openapi-generator`. Принцип в команде — **OpenAPI-first для валидации**: все правила входных DTO формулируются в OpenAPI YAML, codegen превращает их в Jakarta-аннотации автоматически. Java-код только подключает `@Valid` на параметре контроллера и (при необходимости) добавляет custom constraints, которых нет в OpenAPI.
...
- **R-VLD-OAS-4.** Контроллер обязательно `implements <Tag>Api` (generated interface), не `@RestController` с handcrafted маппингом. Это гарантирует что generated `@Valid`/Jakarta-аннотации применяются (см. `rest-api/operation-id-and-tags` REST guide).
  ```java
  @RestController
...
- **R-VLD-OAS-6.** **Двойной контракт generated vs UseCase.** Generated DTO (`CreateOrderRequest`) валидируется через `@Valid` на контроллере. После маппинга в UseCase command (`CreateOrderCommand` record) — **повторная валидация не делается**. Команда пришла «уже чистой» из контроллера. Domain-инварианты (`validation/domain-invariants-in-aggregate`) — отдельный концерн на агрегате, не Jakarta.
...
- **R-VLD-OAS-X5.** Handcrafted DTO в `jsonbean/` или подобном пакете для inbound REST API. Это нарушение `rest-api/operation-id-and-tags`/`spring-bootstrap/external-dtos-are-handcrafted` — все request/response DTO генерируются из OpenAPI. Если видишь `class CreateOrderRequest` без `@Generated` — вырезать, перенести в YAML.
...
- **R-VLD-CFG-X1.** `@ConfigurationProperties` без `@Validated` (см. `validation/config-validated-at-startup`).
...
- **R-VLD-MSG-1.** `message` в аннотации — на русском, для пользователя (см. `rest-api/localization-via-accept-language`).
  ```java
...
|---|---|---|
| Manual `if (cmd.x < 0) throw` в Handler для входной валидации | `validation/input-validated-at-edge` | `@Valid` на контроллере |
| Дублирование Jakarta на UseCase command | `validation/no-revalidation-after-edge` | один раз на контроллере |
| `@ConfigurationProperties` без `@Validated` | `validation/config-validated-at-startup`, `validation/config-validated-at-startup` | `@Validated` на классе |
| Доменный инвариант через `@Min` на поле Aggregate | `validation/domain-invariants-in-aggregate` | бросание domain exception в методе |
| `@NotNull` на примитиве | `validation/no-false-or-composite-constraints` | проверка не нужна, либо тип `Long`/`Integer` |
| Кастомный regex для email вместо `@Email` | `validation/standard-constraints-preferred` | `@Email` |
| Composite-аннотация `@NotBlankAndAtMost50` | `validation/no-false-or-composite-constraints` | две отдельные аннотации |
| Custom validator возвращает `false` для null | `validation/custom-constraint-null-safe` | `isValid(null) → true` + комбинация с `@NotNull` |
| Inner-аннотация в одном файле с DTO | `validation/custom-constraint-is-reusable` | в `core/validation/` или `common/validation/` |
| `@AssertTrue isDateRangeValid()` метод в DTO | `validation/cross-field-rules-on-object`, `validation/custom-constraint-is-reusable` | class-level `@DateRange` |
| Cross-field валидация в Handler | `validation/cross-field-rules-on-object` | class-level constraint на DTO |
| Validation groups «строгая/мягкая» вместо разных DTO | `validation/scenario-groups-are-narrow` | разные DTO |
| Цепочки `@Validated({OnCreate, OnConfirm, OnPay})` | `validation/scenario-groups-are-narrow` | разбить класс |
| Дописывание `@Valid`/`@NotNull` в generated DTO | `validation/generated-artifacts-immutable` | constraints в OpenAPI YAML |
| `useBeanValidation = false` | `validation/generation-carries-constraints` | `true` обязательно |
| Class-level constraint на generated DTO | `R-VLD-OAS-X3` | wrapper-class в коде проекта |
| Дублирование validation в YAML и в Java | `validation/single-source-of-truth` | OpenAPI YAML — единственный источник |
| Handcrafted request DTO без OpenAPI | `validation/controller-implements-generated-contract` | сгенерировать из YAML |
| `@Value("${prop}")` для required-конфига | `validation/config-validated-at-startup` | `@ConfigurationProperties` typed |
| Английский в `message` | `validation/message-in-user-language` | русский |
| Технические термины в message | `validation/message-in-user-language` | пользовательский язык |
| `@Valid` забыт на nested-DTO | `validation/nested-validated-recursively` | `@Valid` на nested-поле |
...
- **`rest-api/validation-errors-list-violations`/`rest-api/validation-errors-list-violations`** (REST API) — формат `violations` в ProblemDetails. Spring `@RestControllerAdvice` ловит `MethodArgumentNotValidException` / `ConstraintViolationException` и формирует ответ — не пиши свой обработчик, используй стандарт.
- **`R-OAS-*`** (REST API) — OpenAPI-first. Constraints живут в YAML, генерируются в DTO.
- **`resilience/client-generated-from-contract`** (Resilience) — для outbound-клиентов `useBeanValidation=true` тоже включён.
- **`R-ENT-*`/`R-AGG-*`** (DDD tactical) — domain invariants. Не путать с входной валидацией.
- **`auth-patterns/money-commands-need-idempotency-key`** (Auth Patterns) — `Idempotency-Key` валидация: формат через `@Pattern` или custom constraint.
