# Caching — индекс правил

> **Что это.** Сжатый индекс правил `caching-style-guide.md`: код + формулировка, по разделам. Рабочий вход
> для скиллов — review цитирует код в findings, design сверяется по чек-листу. **Полная версия
> с примерами, code-блоками, обоснованием и под-пунктами — `caching-style-guide.md`**; открывай её точечно по
...
- **R-CACHE-CFG-4.** Cache settings — через `@ConfigurationProperties` (см. `R-VLD-CFG-1`): В `application.yml`:
...
- **R-CACHE-KEY-4.** Custom `KeyGenerator` — только для **очень сложных** ключей (где SpEL нечитаем). Реализация: Использование: `@Cacheable(keyGenerator = "complexKeyGenerator")`.
**MUST NOT:**
...
- **R-CACHE-INV-3.** При **доменных событиях** (`UserUpdatedEvent`) — invalidation через `@EventListener`: Это паттерн «invalidation as side-effect of domain event», независимый от того, какой именно use case вызвал изменение.
...
- **R-CACHE-PATTERN-3.** **Refresh-ahead** — для критичных hot-данных (главная страница, top-100 продуктов). Реализуется через `@Scheduled` job, который перезаливает cache до истечения TTL: Гарантирует «no cache miss for hot keys».
**MUST NOT:**
...
## 7. Cache stampede
**MUST:**
- **R-CACHE-STAMP-1.** Для **локального кеша** (`ConcurrentMapCacheManager`) — `sync = true`: Spring блокирует все параллельные вызовы на одном ключе до завершения первого.
