...
Превращаем в `@RequiredArgsConstructor`. Если есть валидация в ctor — оставляем явный конструктор (без комментариев, `JS-7.1`).
...
### `JS-7.1` Комментариев в коде нет — вообще
...
### `JS-7.2` Неочевидный WHY выражается именем, структурой или спекой — не комментарием
...
- Не-очевидные **инварианты** (срок жизни bean, threadsafety, порядок вызовов) — выражаются именами/аннотациями (`@Scope`, `@ThreadSafe` и т.п.) или фиксируются в спеке/`docs`; **комментарием — нет** (`JS-7.1`).
- Доменный контракт операции — спека в `docs/spec/`.

### `JS-7.X1` ❌ Любой комментарий — `//`, `/* … */`, Javadoc

Любой комментарий в коде — нарушение `JS-7.1`. `/** … */` с `@param`/`@return`/`@throws`/`@see` — частный случай (Javadoc, `JS-7.5`). Нужен контракт/нюанс — в спеку, не в код.
