# REST API Style Guide — индекс правил

> **Что это.** Сжатый индекс всех правил `rest-api-style-guide.md`: код + формулировка,
> сгруппированы по разделам. Это рабочий вход для скиллов — review цитирует код,
> design сверяется по чек-листу. **Полная версия с примерами, code-блоками и
> обоснованием — `rest-api-style-guide.md`**; читай её точечно по конкретному разделу,
> когда нужны детали, а не целиком.
>
> Коды: `R-<SECTION>-<N>` — «Обязательно», `R-<SECTION>-X<N>` — «Запрещено».
> Разделы: PRIN, URL, RES, MTH, NEST, ALIAS, ACT, VER, QRY, FLD, RSP, HDR, ERR,
> RATE, FILE, DEP, BATCH, ASYNC, LOC, OAS.
