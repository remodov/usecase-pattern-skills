...
excerpt: "Универсальный шаблон спецификации сервиса: 16 разделов, четыре уровня детализации (Tier 0 — as-is снимок существующего сервиса, Tier A — legacy, Tier B — UseCase Pattern, Tier C — DDD/Hexagonal). Подходит для любого стиля архитектуры, включая brownfield-онбординг."
keywords: "Use Case спецификация, шаблон спецификации, DDD spec, Bounded Context, бизнес-правила, команды, события, Tier 0 A B C, brownfield"
...
## Уровни спеки (Tier 0 / A / B / C)

Глубина спеки зависит от архитектурного стиля сервиса и того, **откуда берётся вход**. Не каждый сервис — DDD; для существующего легаси-кода полная DDD-спека избыточна, а до неё ещё надо собрать честный снимок «как есть». Поэтому шаблон делится на четыре уровня — **Tier**:

| Tier | Архитектура | Источник входа | Скилл-генератор | Привязка к [UCP-уровням](/use-case-pattern/) |
|---|---|---|---|---|
| **0. As-is снимок** | любая (существующая) | код репо (Java, миграции, `application.yml`, OpenAPI) | `ucp-spec-tier-0` | — |
| **A. Слоёная / уже существующая** | Controller → Service → Repository, без библиотеки `usecase-pattern` | бизнес-бриф + Event Storming | `ucp-spec-design` | — |
| **B. UseCase Pattern** | подключена `usecase-pattern`, есть `UseCase` + `UseCaseHandler` (с CQRS-маркерами или без) | бизнес-бриф + Event Storming | `ucp-spec-design` | UCP **Level 1–2** |
| **C. DDD / Hexagonal** | + агрегаты, доменные события, ports/adapters | бизнес-бриф + Event Storming | `ucp-spec-design` | UCP **Level 3–4** |

**Tier 0 — отдельный класс спеки.** В отличие от A/B/C, его пишет не аналитик с архитектором, а скилл `ucp-spec-tier-0`, который читает кодовую базу и собирает снимок «что есть сейчас». Содержимое — факт из кода, миграций и конфигов. **Все 16 разделов присутствуют формально**, чтобы downstream-скиллы (`ucp-ddd-tactical-review`, `ucp-cqrs-review` и др.) могли запускаться против Tier 0 без особого режима. Поля, для которых в репо нет данных (не объявлены агрегаты, нет ABAC-условий, нет SLA), заполняются литералом `not-declared` — это явный gap-сигнал для парсера и для человека, а не пустота.

Tier 0 не претендует на UCP-семантику. Он не пытается восстановить агрегаты там, где код anemic, не выдумывает domain events, не классифицирует «правильно / неправильно». Это база для онбординга нового инженера и точка отсчёта для последующего рефакторинга в Tier A/B/C.

Каждый раздел шаблона помечен минимальным Tier-ом, на котором он становится обязательным:

- 🟫 **0** — формально присутствует во всех спеках, включая as-is снимок; неприменимые поля = `not-declared`.
- 🟦 **A** — содержательно обязательно для всех систем, включая существующие.
...
| Раздел | Tier 0 (as-is) | Tier A | Tier B | Tier C |
|---|---|---|---|---|
| 1. Bounded Context | модуль/сервис из README+entry-points | ⭕ как «модуль» | ✅ | ✅ |
| 2. Ubiquitous Language | глоссарий из имён таблиц / `@Entity` / топиков | ✅ | ✅ | ✅ |
| 3. Domain Model | DDL из миграций + `@Entity` + FK | только ER + таблицы | + UseCase-модели | + агрегаты, VO, события |
| 4. Жизненный цикл | enum-статусы из кода | если есть статусы | ✅ | ✅ |
| 5. Роли и права | `@PreAuthorize` + `SecurityConfig` или `not-declared` | ✅ | ✅ | ✅ |
| 6. Бизнес-правила | `@Valid` + БД-constraints + проверки в коде + утверждения тестов | ✅ | ✅ | ✅ |
| 7. Commands | HTTP-мутирующие эндпоинты + `@KafkaListener` | список операций | UseCase-команды | + предусловия / события |
| 8. Domain Events | Kafka-publish из `KafkaTemplate.send` или `not-declared` | ❌ | ⭕ если есть Kafka | ✅ |
| 9. Queries / Read Model | GET-эндпоинты + read-only методы | список read-операций | + Read Model на L2 | ✅ |
| 10. Use Cases | реконструкция из контроллер-методов + интеграционных тестов | сценарии | + ссылки на UseCase | ✅ |
| 11. UI | OpenAPI / Swagger или `not-declared` | ✅ | ✅ | ✅ |
| 12. Saga | `@Transactional` поверх нескольких ресурсов + outbox-таблицы или `not-declared` | ❌ | ⭕ редко | ✅ |
| 13. Каталог ошибок | `@ExceptionHandler` + коды | ✅ | ✅ | ✅ |
| 14. Интеграции | БД + Kafka + REST-клиенты + S3 из `application.yml` | базовые контракты | + UseCase как порты | + Context Mapping типы |
| 15. Критерии приёмки | условия из существующих интеграционных тестов | ✅ | ✅ | ✅ |
| 16. НФТ | timeouts / pool sizes из `application.yml` + alert-rules или `not-declared` | ✅ | ✅ | ✅ |

> ✅ обязательно · ⭕ упрощённо / опционально · ❌ не пишется · `not-declared` = в репо данных нет (валидный литерал для всех обязательных полей)

В заголовке вашей спеки явно укажите Tier: «**Order Service — DDD-спецификация (Tier C)**» или «**Legacy Billing — as-is спецификация (Tier 0)**». Это сразу задаёт читателю ожидания от глубины разделов и от стиля содержимого (написано человеком vs реверс-инжиниринг из кода).

### Литерал `not-declared`

Все обязательные поля frontmatter (см. ниже «Per-item cards») присутствуют во всех тирах. Когда данных для поля в источнике нет, в значение пишется литерал `not-declared` — никогда не пустая строка, не `null`, не пропуск ключа. Это инвариант spec-as-code: парсеры (Dataview, downstream-скиллы, AI-ingestion) распознают `not-declared` как явный gap и не падают.

Литерал применяется на любом уровне, но реально часто встречается только в Tier 0. В Tier A/B/C `ucp-spec-design` использует его как fallback, когда даже после уточнений у бизнес-аналитика данных нет (например, нет SLA на §16 NFR).
