...
lang: any
track: any
description: Спроектировать новый сервис на платформенном уровне — карточка services/<name>/README.md, запись в _registry.yaml, обновление context-map и data-ownership, проверка конфликтов ownership и UL. Per-service спеку и репо не создаёт.
when_to_use: Триггеры — «новый сервис на платформе», «выделяем BC», «добавить сервис в архитектуру». Только из корня architecture/.
...
- **`contracts/events/_index.md`** — реестр событий корпуса, обязательный артефакт (`arch/one-publisher-per-event`): каналы нового сервиса добавляются строками сюда.
- **ADR:**
  - `docs/adr/0001-tier-per-service.md` — что такое tier 1/2/3.
  - `docs/adr/0002-no-direct-data-edits.md` — owner-edits-only.
- **Стиль-гайды:**
  - `.claude/docs/shared/arch/spec.md` — правила `R-ARCH-CTX-*`, `R-ARCH-UL-*`, `R-ARCH-DATA-*`, `arch/one-publisher-per-event` (реестр событий), `arch/index-documents-are-tables` (индексные документы — таблицами).
- **Парные скиллы:**
...
5. **Какие данные владеет** — список сущностей. Сверить с `docs/03-data-ownership.md` — не должно пересекаться с уже заявленными у других сервисов (`arch/single-owner-per-entity`).
6. **Связи с существующими сервисами** — для каждой связи:
   ...
   - Если shared-kernel — требуется ADR (`arch/shared-kernel-needs-adr`).
7. **Каналы (топики), которые сервис публикует и слушает** — для каждого: имя топика/канала, брокер (Kafka / RabbitMQ / …), тип сообщения, кто предполагаемые consumers. Если на этом этапе что-то ещё не решено — так и фиксируй, дальше это ляжет в реестр литералом `not-declared`, а не потеряется. Если сервис вообще не работает с событиями — так и скажи, шаг реестра пропускается.
8. **Участие в существующих BP** — какие из BP-01..BP-NN затрагивают новый сервис; в какой роли (orchestrator / participant / publisher / consumer).
9. **Новые BP, которые этот сервис вводит** — список (детали — потом через `ucp-arch-bp-design`).

Прежде чем переходить дальше — подтверди понимание у пользователя.

### Шаг 3: Проверка конфликтов

- **`arch/single-owner-per-entity`**: сущности из Шага 2.5 — проверить `grep`'ом по `docs/03-data-ownership.md`. Если уже владеет кто-то — ❌, обсудить с пользователем.
- **`arch/single-owner-per-entity`**: имена агрегатов — `grep` по секциям «Домен-агрегаты» во всех существующих карточках. Если коллизия — либо переименовать, либо в `docs/02-ubiquitous-language.md` пометить как omonym.
- **`arch/ubiquitous-language-is-consistent`**: новые термины из спеки сервиса (понадобятся позже) — пока пометка для `ucp-spec-design`.
- **`arch/shared-kernel-needs-adr`**: shared-kernel — требует ADR. Если пользователь выбрал — спросить почему, и оформить рекомендацию открыть `docs/adr/0006-...md`.
- **`arch/one-publisher-per-event` / `arch/one-publisher-per-event`**: каналы из Шага 2.7 — проверить `grep`'ом по `contracts/events/_index.md`, что топик с таким именем ещё не заявлен другим publisher'ом. Совпадение — ❌, обсудить: либо это тот же канал (тогда новый сервис — consumer, не publisher), либо имя надо развести.
...
### Шаг 8: Добавить каналы сервиса в реестр событий `contracts/events/_index.md`

Реестр событий — **обязательный артефакт корпуса** (`arch/one-publisher-per-event`). Если сервис публикует или слушает хотя бы один канал (Шаг 2.7) — его строки появляются здесь **сразу**, на платформенном уровне, а не потом, когда напишут AsyncAPI: иначе между дизайном и первым `/ucp-arch-sync` в корпусе нет ответа на вопрос «какие каналы есть в системе».

Если файла нет — создай его с шапкой и таблицей. Заголовок таблицы — ровно этот, колонки не переставляй и не переименовывай (на них завязан детект `ucp-arch-consistency-review`); свои дополнительные колонки — только справа от шести обязательных:

```markdown
| Топик/канал | Publisher (сервис) | Consumers | Брокер | Тип сообщения | Контракт (ссылка на asyncapi) |
|---|---|---|---|---|---|
| `<топик>` | <новый сервис> | <consumers> | Kafka | <MessageType.v1> | not-declared |
```

Заполнение на этом этапе:

- **Publisher** — новый сервис для каналов, которые он публикует. Для каналов, которые он только слушает, строка либо уже есть (тогда допиши сервис в колонку `Consumers`), либо публикует её кто-то другой — тогда это расхождение, вынеси в чат, не выдумывай publisher'а.
- **Контракт** — `not-declared`, пока нет `services/<name>/contracts/asyncapi.y*ml`; ссылка появится после первого `/ucp-arch-sync`.
- Любая другая неизвестная ячейка — тоже литерал `not-declared`. **Пустых ячеек не оставляй** (`arch/index-documents-are-tables`): пустая ячейка неотличима от «забыли колонку», `not-declared` честно говорит «не объявлено» и ищется grep'ом.
- Строки чужих сервисов не трогай.

Если сервис не работает с событиями — шаг пропускается, скажи об этом явно в финальной сводке.

### Шаг 9: Опционально обновить `docs/00-overview.md`
...
- **После имплементации:** `/ucp-arch-sync` подхватит спеку и контракты и перестроит реестр событий — `not-declared` в колонке «Контракт» заменится ссылкой на asyncapi.
...
Каналы в реестре событий: <список топиков или «нет»>
Участие в BP: <список>
```

### Шаг 11: Disambiguation reminder
