...
lang: any
track: any
description: Оценить влияние планируемого изменения в архитектурном корпусе (read-only) — классификация breaking/compatible/semantic-break, список consumer'ов, миграционная стратегия (expand-contract для API, vN+1 для событий, ADR для UL).
when_to_use: Триггеры — «что сломается если», «можно ли удалить», «кого затронет изменение», «impact analysis». Только из корня architecture/.
