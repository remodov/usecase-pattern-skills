---
name: ucp-arch-query
description: Read-only навигация по архитектурному корпусу — отвечает на вопросы «кто публикует событие X», «какие сервисы трогают Y», «через какие BP проходит Z», «сервисы tier=3», «события order», «какие сервисы используют saga», «что в context-map про catalog». Ничего не меняет в файлах. Если вопрос про эволюцию («что будет если...») — рекомендует ucp-arch-impact. Триггеры: «кто публикует», «какие сервисы», «через какие BP», «найди в архитектуре».
