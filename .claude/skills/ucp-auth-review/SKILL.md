...
4. **Формат finding, локализация, серьёзность, резюме, запрет правок** — см. `.claude/docs/review-finding-format.md` (правила `RFF-1`..`RFF-16`). Перед каждым finding обязательна Read-проверка строки (`RFF-1`..`RFF-5`), поле `Строка` в формате обязательно (`RFF-7`).
