...
You are designing or scaffolding a new domain model: a bounded context, an aggregate, a value object, or a domain event. The implementation must follow the team's DDD Tactical Patterns Style Guide and use the abstractions from the `ddd-building-blocks` library (package `ru.mosmetro.ddd`).
...
2. **Confirm the library is available.** Check `build.gradle` / `pom.xml` for `ru.mosmetro:ddd-building-blocks`. If absent, instruct the user to add it (and offer the dependency snippet) — do not invent local copies of `Entity`/`AggregateRoot`/`ValueObject`.
