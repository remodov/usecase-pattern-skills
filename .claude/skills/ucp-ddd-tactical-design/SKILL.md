...
Ты проектируешь или генерируешь шаблон новой доменной модели: bounded context, агрегат, value object или доменное событие. Реализация должна следовать командному DDD Tactical Patterns Style Guide и использовать абстракции из библиотеки `ddd-building-blocks` (пакет `ru.mosmetro.ddd`).
