...
3. **Определи Tier и Уровень проекта** (см. `usecase-pattern-style-guide.md` §2). Outbound-интеграции уместны на **Уровне 3+** (есть domain port в `core/`). На Уровне 1–2 — `<System>Client` инжектится в `<Operation>UseCaseHandler` напрямую, без port-абстракции. Если Уровень 1–2 — упрости вывод (без отдельного `<System>Port`-интерфейса).
