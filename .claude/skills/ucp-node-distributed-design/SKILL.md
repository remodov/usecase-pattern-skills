...
lang: node
description: Спроектировать распределённый сценарий в NestJS-микросервисах по UCP — saga orchestration/choreography со state-таблицей и сквозным sagaId, idempotency, outbox+inbox, compensation, eventual consistency, запрет 2PC/XA.
...
1. **Прочитай** требования `node-style/*`. Коды в обосновании, не в коде. Связанные: `backend/kafka/node/...` (outbox/idempotent consumer), `cqrs` (EC read-model), `backend/usecase-pattern/node/...` (граница TX), `pg-runtime` (saga/idempotency-таблицы).
...
- Saga для одного сервиса (`distributed/patterns-only-when-crossing-services`); saga без compensation (`distributed/every-step-has-compensation`/`distributed/every-step-has-compensation`); saga state in-memory (Map/RxJS-стейт, `distributed/saga-state-is-persistent`); saga в одном handler с use case (`distributed/saga-separate-from-use-cases`).
...
- 2PC/XA (`distributed/no-two-phase-commit`); multi-DataSource-commit-цепочка (`distributed/no-two-phase-commit`); прямой `producer.send` без outbox (`distributed/outbox-for-outgoing-events`).
