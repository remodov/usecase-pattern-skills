...
2. **Confirm the library is available.** Check `build.gradle` / `pom.xml` for `ru.mosmetro:usecase-pattern-starter`. If absent, instruct the user to add it (and offer the dependency snippet) — do not invent local copies of `UseCase` / `UseCaseHandler` / `UseCaseDispatcher`.
...
5. **Produce the code.** Write complete Java files (Java 21+). **Lombok-defaults обязательны** (`JS-6.1`–`JS-6.7` в `java-style-guide.md`): `@RequiredArgsConstructor` на каждом Spring-бине с DI, `@Slf4j` вместо ручного `Logger`, `@Getter` на доменных исключениях с payload-полями. Lombok **не** навешиваем на records.
   ...
   **Не цитировать коды правил в комментариях кода** (`JS-7.3`). Никаких `// R-UC-3`, `// R-LAY-2`, `// R-DSP-X2`, `// R-CQRS-1` в исходниках. Соответствие правилу выражается именами (`CreateProductUseCase`, `*Query*Handler`) и структурой (record + marker interface + `@Component` + `@Transactional`). Комментарий уместен только если WHY неочевиден из кода — и тогда без кода правила.
   ...
   - **Controller** — `@RestController` `class XController implements <Tag>Api` (style-guide §12.2 — интерфейс генерируется openapi-generator'ом из `src/main/resources/openapi/<service>.openapi.yaml`). Методы — `@Override` интерфейсных, дополнительно навешиваются `@PreAuthorize`. Body метода: маппинг request DTO → UseCase, `dispatcher.dispatch(...)`, оборачивание в `ResponseEntity`. Никакого `@RequestMapping` на классе, никаких handcrafted request/response DTO в `jsonbean/`. Если openapi-generator не подключён — это бэйлаут к `ucp-bootstrap-design`, не повод писать ручной контроллер.
