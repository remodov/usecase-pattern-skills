...
Ты ревьюишь Java/Spring-код на соответствие Use Case Pattern Style Guide и правильное использование библиотеки `usecase-pattern` (пакеты `ru.mosmetro.usecase`, `ru.mosmetro.usecase.cqrs`).
...
3. **Определи объект ревью.** Если пользователь назвал файлы — бери их. Иначе:
   ...
   - Проверь `build.gradle` / `pom.xml` на `ru.mosmetro:usecase-pattern-starter` (или `usecase-pattern`). Если нет — замечание уровня **Замечание**, но продолжай.
