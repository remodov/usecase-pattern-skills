...
lang: python
description: Сгенерировать persistence-слой на async SQLAlchemy 2.0 + Liquibase из доменного порта по UCP (коды R-SQLA-*) — SqlAlchemy<X>Repository реализует Protocol из core/, маппер ORM↔domain, UoW, select() 2.0 + eager-load, ViewRepository, Liquibase changelog.
...
4. **Граница транзакции — НЕ в репозитории** (`commit` на Handler через UoW, `R-SQLA-SESS-1`). Маппинг в домен — до `commit` (`R-SQLA-SESS-X2`). Обработка ошибок в TX: контекст шага через `bind_contextvars(step=..., use_case=...)`, исключение не глотать (авто-rollback + edge-лог ошибки и шага) — `R-SQLA-SESS-4`/`X3`.
...
- `text(f"...{x}")` с конкатенацией (`R-SQLA-QRY-X4`); `create_all()` в проде вместо Liquibase.
