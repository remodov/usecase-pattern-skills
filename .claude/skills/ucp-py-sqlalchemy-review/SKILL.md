...
lang: python
description: Ревью persistence-слоя на async SQLAlchemy 2.0 + Liquibase по UCP (коды R-SQLA-*) — порт/маппер ORM↔domain, граница TX на Handler через UoW, select() 2.0, ViewRepository, типы Decimal/tz/UUID.
...
3. **Прогон.**
   ...
   - **Миграции:** схема через Liquibase, не `create_all()` в проде (`R-SQLA-MIG-1`, cross-ref `PYBOOT-X4`); безопасность по `PG-M-*` (`R-SQLA-MIG-2`); сгенерированный changelog вычитан руками (`R-SQLA-MIG-3`). Правка применённого changeset → `R-SQLA-MIG-X1`.
...
6. **Серьёзность** (`RFF-12`):
   ...
   - **Предупреждение** — бизнес-логика в репозитории (`R-SQLA-REPO-X2`), доменная логика на ORM (`R-SQLA-MODEL-X1`), `session.query(...)` legacy (`R-SQLA-QRY-X1`), `__dict__`-маппинг (`R-SQLA-MAP-X1`), ORM-объект после commit (`R-SQLA-SESS-X2`), `fetchall()` без `LIMIT` (`R-SQLA-QRY-X3`), проглатывание исключения/partial commit в транзакции (`R-SQLA-SESS-X3`), правка применённой ревизии (`R-SQLA-MIG-X1`).
