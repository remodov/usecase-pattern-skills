...
2. **Determine the Tier** for the service (template §«Уровни спеки»):
   ...
   Inspect the project for hints (Java imports of `ru.mosmetro.usecase.*`, presence of `core/` + `adapter/`, `Aggregate*`, `Entity<ID>`). If still unclear, **ask the user** which Tier they want before writing.
