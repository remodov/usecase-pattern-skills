...
2. **Определи Tier** для сервиса (шаблон §«Уровни спеки»):
   ...
   Осмотри проект на признаки (Java-импорты `ru.vikulinva.usecase.*`, наличие `core/` + `adapter/`, `Aggregate*`, `Entity<ID>`). Если по-прежнему непонятно — **спроси у пользователя**, какой Tier он хочет, прежде чем писать.
