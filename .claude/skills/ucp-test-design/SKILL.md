...
description: Design integration and unit tests for a Java/Spring service following the team's Test Strategy — synchronous tests, only PostgreSQL via Testcontainers + WireMock for external HTTP, no Kafka/Redis in base, deterministic time and UUIDs via @MockitoBean, fluent DatabasePreparer + TestObjectGenerator. Use when adding tests to a new UseCase / Handler or onboarding a module to the team's testing approach.
allowed-tools: Read Glob Grep Write Edit Bash(./gradlew*) Bash(mvn*) Bash(git diff*)
---

# Test Design

You are writing tests for a Java/Spring service following the team's Test Strategy.

## Зависимости

- **`.claude/docs/test-strategy.md`** в проекте (или из `claude-code-java`) — единственный источник правил. Каждое правило имеет код `TS-N`.
...
10. **Перед презентацией кода** прогони чек-лист (`TS` § 9):
...
1. **Summary**: что тестируем, какой слой, какие use case-ы и BR покрыты.
2. **Файловое дерево** новых тестов и хелперов.
3. **Каждый файл** в отдельном code block с путём.
4. **Implementation notes**: что нужно в `build.gradle` (Testcontainers, WireMock, JUnit Jupiter, AssertJ, BDDMockito), как пустить локально (`./gradlew test --tests *.OrderConfirmIntegrationTest`).
