...
Базовый материал статьи: <https://vikulin-va.ru/use-case-pattern/>
и <https://vikulin-va.ru/use-case-pattern/library/>.
...
- **R-LAY-3.** Маппинг между слоями — через **MapStruct** (генерируется,
  не пишется руками), либо явные мапперы в виде `@Component`-ов.
