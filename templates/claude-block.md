...
**Загружай спеку в контекст в начале сессии, не в конце.** Прежде чем
начинать работу над сервисом — прочитай spec-landing (и нужные per-item
карточки). Если читать спеку поздно, после десятков других tool-вызовов,
prompt-кэш churn-ится и каждый последующий ход стоит дороже. Раннее
чтение = спека оседает в тёплой части кэша на всю сессию.

**Авто-загрузка через `@import`** (опционально): если хочется, чтобы
spec-landing подтягивался автоматически в каждый контекст — добавь
**снаружи** managed-блока (выше `<!-- BEGIN ucp-skills -->` или ниже
`<!-- END ucp-skills -->`) строку:
