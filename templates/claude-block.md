...
для независимых задач**. Для design-цепочки и рефакторинга — отказываться,
делать последовательно.

| Ситуация | Параллельно? |
|---|---|
| Прогнать N ревью-скиллов на одном PR (`ucp-pattern-review` + `ucp-api-review` + `ucp-java-style-review` + `ucp-ddd-tactical-review` + `ucp-auth-review`) | да |
...
Параллельный режим хорош для **fan-out** (одна задача × N входов) и
**fan-in ревью** (N независимых проверок одного кода). Для зависимых
шагов — последовательно.
